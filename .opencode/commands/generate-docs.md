...
Generates a documentation structure in the `docs` folder for shared-ai-configs, adapted to the current repository or project, regardless of type (microservice, app, monorepo, etc.).
...
2. **Decide where to create documentation**
   ...
   - Alternative: if `.opencode/` exists, it may use `.opencode/docs/` to keep docs close to agent configuration.
   - The agent must create the folder if it does not exist.

3. **Create the folder and file structure** (inspired by `shared-ai-configs/.opencode/docs/`):
...
- **References**: in projects that use `shared-ai-configs`, the agent may reference `shared-ai-configs/.opencode/docs/` as structure reference, but generated content must be specific to the current repository.
...
- `shared-ai-configs/.opencode/docs/` (overview, architecture, guides)
- `shared-ai-configs/.opencode/docs/projects/` (README, _TEMPLATE, project folders with OVERVIEW, DETAILED_INFORMATION, KNOW-HOW, BUSINESS-TO-CODE)
