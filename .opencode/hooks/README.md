# Hooks - the coding agent Automation Configuration

## Description

This folder contains **documentation about the coding agent hooks**. Hooks are configured directly in `.opencode/settings.json` and allow you to automate tasks that run at specific moments in the lifecycle of a the coding agent session.
...
Note: Hooks are configured in .opencode/settings.json, not as separate scripts.
```

## Configured Hooks

Hooks are defined in `.opencode/settings.json`. Currently, the project has the following hook configured:

### 1. **Cleanup NUL Files** (Stop Hook)

**Description**: Hook automatically executed at the end of each the coding agent session (`Stop` event).
...
**When it runs**: Automatically on the the coding agent `Stop` event (session end).
    ...
    "Stop": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "powershell.exe -ExecutionPolicy Bypass -File \"c:\\Users\\[REDACTED]\\Repositories\\the-project\\shared-ai-configs\\.opencode/\hooks\\cleanup-nul-files.ps1\""
          }
        ]
      }
    ]
...
**Note**: The PowerShell script path above is an the-project example. Adjust it only if your local the-project workspace path is different.

## Configuration

### Where to Configure Hooks

Hooks are configured directly in the project file `.opencode/settings.json`. It is not necessary to create separate scripts in the `hooks/` folder.
...
Common the coding agent events where you can configure hooks:

- **`SessionStart`**: Start of a new session
- **`Stop`**: End of a session (when the coding agent is closed)
- **`FileChanged`**: When a file is modified
- **`BeforeCommand`**: Before running a command

See the official the coding agent documentation for the full list of available events.

### Hook Types

- **`command`**: Runs a system command (PowerShell, bash, etc.)
- Other types may be available depending on the the coding agent version

### Example: Add a New Hook

To add a new hook, edit `.opencode/settings.json`:
...
2. **the coding agent detects** the event configured in `settings.json`
...
1. Check `.opencode/settings.json` to confirm the hook is configured
2. Run the action that triggers the event (e.g., close the coding agent to test the `Stop` hook)
...
4. See the coding agent documentation for troubleshooting

## Add New Hooks

To add a new hook:

1. **Edit `.opencode/settings.json`** and add the hook configuration
...
2. **Performance**: Keep hooks fast. They should not block the coding agent operations
...
- **Configuration**: `.opencode/settings.json` - file where hooks are configured
- **AGENTS.md**: Main repository documentation
- **the coding agent Docs**: Official the coding agent documentation for hooks and configuration
