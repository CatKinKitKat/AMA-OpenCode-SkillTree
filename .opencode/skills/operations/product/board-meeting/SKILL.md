...
1. Load `~/.opencode/company-context.md`
2. Load Layer 2 approved decisions from `~/.opencode/decisions/approved/` **(Layer 2 ONLY: never raw transcripts)**
...
- **Layer 1:** Write full transcript → `~/.opencode/decisions/raw/YYYY-MM-DD-<slug>.md`
- **Layer 2:** Write approved decision record → `~/.opencode/decisions/approved/YYYY-MM-DD-<slug>.md` and append to the index `~/.opencode/decisions/approved/decisions.md`
...
Migration: a legacy `memory/board-meetings/` folder may exist from earlier versions. Read it for history but write new transcripts and decisions to `~/.opencode/decisions/`.: -
...
| Layer contamination | Phase 1 loads `~/.opencode/decisions/approved/` only: hard rule |: -
