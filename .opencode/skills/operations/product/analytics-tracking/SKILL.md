...
If `.opencode/product-marketing-context.md` exists, read it before asking questions. Use that context and only ask for what's missing.
...
| `page_title` | string | `Pricing - the-project` | Auto-captured by GA4 |
...
For funnels that cross domains (e.g., `the-project.com` → `app.the-project.com`):
