#!/usr/bin/env bash
# GitHub environment detection helper for the agent skills.
#
...
if command -v gh &>/dev/null && gh auth status &>/dev/null 2>&1; then
    ...
    GH_AUTH_METHOD="curl"
elif [ -f "$HOME/.agent/.env" ] && grep -q "^GITHUB_TOKEN=" "$HOME/.agent/.env" 2>/dev/null; then
    GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" "$HOME/.agent/.env" | head -1 | cut -d= -f2 | tr -d '\n\r')
