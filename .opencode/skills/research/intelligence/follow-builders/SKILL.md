...
- **Other** (the coding agent, Cursor, etc.): Non-persistent agent. Terminal closes = agent stops.
...
**If non-persistent agent (the coding agent, Cursor, etc.):**
...
```bash
cd ${OPENCODE_SKILL_DIR}/scripts && node prepare-digest.js 2>/dev/null
```
...
cd ${OPENCODE_SKILL_DIR}/scripts && node deliver.js --file /tmp/fb-digest.txt 2>/dev/null
```
...
cp ${OPENCODE_SKILL_DIR}/prompts/<filename>.md ~/.follow-builders/prompts/<filename>.md
```
