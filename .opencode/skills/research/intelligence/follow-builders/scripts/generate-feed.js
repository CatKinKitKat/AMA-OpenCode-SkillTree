...
// Scrapes the The agent Blog index page (the agent.com/blog).
// This is a Webflow site. We extract article links, titles, and dates
// from the HTML structure.
function parseThe agentBlogIndex(html) {
  const articles = [];
  const seenSlugs = new Set();
  ...
  while ((linkMatch = linkRegex.exec(html)) !== null) {
    ...
    articles.push({
      title: "", // Will be filled when we fetch the article page
      url: `https://the agent.com/blog/${slug}`,
      publishedAt: null,
      description: "",
    });
  }
...
// Extracts the main text content from a The agent Blog article page.
// Uses JSON-LD schema data if present, then falls back to the rich text body.
function extractThe agentBlogArticleContent(html) {
  let title = "";
  let author = "";
  let publishedAt = null;
  let content = "";
...
async function fetchBlogContent(blogs, state, errors) {
  ...
  for (const blog of blogs) {
    ...
    try {
      ...
      } else if (blog.indexUrl.includes("the agent.com")) {
        candidates = parseThe agentBlogIndex(indexHtml);
      }
      ...
      for (const article of newArticles) {
        try {
          ...
          } else if (article.url.includes("the agent.com/blog")) {
            extracted = extractThe agentBlogArticleContent(articleHtml);
          }
