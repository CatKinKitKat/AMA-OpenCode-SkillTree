...
version: 1.1.1
author: xdevplatform + openclaw + the agent
license: MIT
...
- Public read-only research without API auth: use `references/public-x-research-without-auth.md`.: -
...
3. If auth is missing entirely, stop for authenticated X actions and direct the user to the "One-Time User Setup" section: do NOT attempt to register apps or pass secrets yourself. For read-only discovery tasks where exact X API access is not required, do not stop: use a public search index for `site:x.com ...` queries, extract candidate posts/accounts, then verify claims against the projects' official pages before reporting.
4. Start authenticated sessions with a cheap read (`xurl whoami`, `xurl user @handle`, `xurl search ... -n 3`) to confirm reachability.
...
- the agent adaptation: reformatted for the agent skill conventions. Safety guardrails preserved verbatim.
