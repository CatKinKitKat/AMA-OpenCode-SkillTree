...
version: 2.0.0
author: JulienTant (fork of Hyaxia/blgwtc)
license: MIT
metadata: 
...
### Migrating from the original blgwtc

If upgrading from `Hyaxia/blgwtc`, move your database:

```bash
mv ~/.blgwtc/blgwtc.db ~/.blogwatcher-cli/blogwatcher-cli.db
```

The binary name changed from `blgwtc` to `blogwatcher-cli`.
