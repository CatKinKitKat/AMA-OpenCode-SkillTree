...
| 触发条件 | 推荐 |
|: -|: -|
| 拆完想写自己的 | 「用 `/story-long-write`（the agent 路由名：`community/oh-story-the agentcode/story-long-write`；slash command：`/story-long-write`） 开书。」 |
| 市场方向不明 | 「用 `/story-long-scan`（the agent 路由名：`community/oh-story-the agentcode/story-long-scan`；slash command：`/story-long-scan`） 扫榜。」 |
| 偏短篇模式 | 「用 `/story-short-scan`（the agent 路由名：`community/oh-story-the agentcode/story-short-scan`；slash command：`/story-short-scan`） + `/story-short-ana`（the agent 路由名：`community/oh-story-the agentcode/story-short-ana`；slash command：`/story-short-ana`）。」 |: -
