...
AGENT_HOME_DIR="${AGENT_HOME:-$HOME/.agent}"
...
# ── 3. Ensure the agent config has twozero_td MCP entry ──
if [[ ! -f "$AGENT_CFG" ]]; then
    echo -e " ${FAIL} the agent config not found at ${AGENT_CFG}"
    manual_steps+=("Create ${AGENT_CFG} with twozero_td MCP server entry")
elif grep -q 'twozero_td' "$AGENT_CFG" 2>/dev/null; then
    echo -e " ${OK} twozero_td MCP entry exists in the agent config"
else
    echo -e " ${WARN} Adding twozero_td MCP entry to the agent config..."
    python3 -c "
...
" 2>/dev/null && echo -e " ${OK} twozero_td MCP entry added to config" \
    ...
    manual_steps+=("Restart the agent session to pick up config change")
fi
