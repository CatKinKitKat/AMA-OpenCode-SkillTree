...
related_skills: [native-mcp, ascii-video, manim-video, agent-video]
---
...
```bash
bash "${AGENT_HOME:-$HOME/.agent}/skills/creative/tchdsg-mcp/scripts/setup.sh"
```

The script will:
1. Check if TD is running
2. Download twozero.tox if not already cached
3. Add `twozero_td` MCP server to the agent config (if missing)
