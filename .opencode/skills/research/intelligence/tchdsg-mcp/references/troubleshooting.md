...
If it's unclear, use `td_read_textport` to see the full TD console output: Python exceptions are always printed there.
...
Use `td_get_perf` to see which operators are slow. Look at cook times: anything over 1ms per frame is worth investigating.
...
## 5. the agent Configuration

### Config location

`$AGENT_HOME/config.yaml` (defaults to `~/.agent/config.yaml` when `AGENT_HOME` is unset)
...
Restart the the agent session for changes to take effect. The MCP connection is
