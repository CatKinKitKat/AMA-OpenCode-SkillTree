...
version: 1.0.0
author: Siddharth Balyan + the agent
license: MIT
...
This skill intentionally does not vendor a separate CLI implementation into the agent. Install and use upstream `x-cli` instead.
...
If you already keep your X credentials in `~/.agent/.env`, the cleanest setup is:

```bash
mkdir -p ~/.config/x-cli
ln -sf ~/.agent/.env ~/.config/x-cli/.env
```
...
- **Credential drift**: if you rotate tokens in `~/.agent/.env`, make sure `~/.config/x-cli/.env` still points at the current file.
