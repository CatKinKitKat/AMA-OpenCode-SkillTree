---
name: story-short-ana
description: |
---


# story-short-ana：短篇网文拆文
...
| 触发条件 | 推荐 |
|: -|: -|
| 拆完想写自己的 | 「结构看懂了，开写。用 `/story-short-write`（the agent 路由名：`community/oh-story-the agentcode/story-short-write`；slash command：`/story-short-write`）。」 |
| 市场方向不明 | 「先看市场。用 `/story-short-scan`（the agent 路由名：`community/oh-story-the agentcode/story-short-scan`；slash command：`/story-short-scan`）。」 |
| 适合做长篇 | 「用 `/story-long-scan`（the agent 路由名：`community/oh-story-the agentcode/story-long-scan`；slash command：`/story-long-scan`） 看市场，再 `/story-long-ana`（the agent 路由名：`community/oh-story-the agentcode/story-long-ana`；slash command：`/story-long-ana`） 拆解。」 |: -
