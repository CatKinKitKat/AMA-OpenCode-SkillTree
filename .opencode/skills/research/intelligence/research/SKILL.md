...
Requires `WebSearch` + `WebFetch` for the fallback workflow. Specialist skills (`pulse`, `grants`, `litreview`, `syllabus`, `patent`, `dossier`) must be present for delegation to work. Node.js with `docx` package required if Q2 = document mode. Works in the coding agent CLI natively. In the agent platform with web tools + Code Execution, the workflow is supported.
