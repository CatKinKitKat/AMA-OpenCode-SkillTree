...
version: 1.0.0
author: the agent + Teknium (design systems sourced from VoltAgent/awesome-design-md)
license: MIT
...
- **`the agent-design`**: use for the design *process and taste* (scoping a brief,
  ...
  after a known brand: `the agent-design` drives the workflow, this skill supplies
...
Each template includes a **the agent Implementation Notes** block at the top with:
...
<head>
  ...
  <!-- Paste the Google Fonts <link> from the template's the agent notes -->
...
for sohne-var), follow the template's weight, size, and letter-spacing values closely: those carry more visual identity than the specific font face.
...
| Template | Site | Style |
|---|---|---|
| `the assistant.md` | Anthropic the assistant | Warm terracotta accent, clean editorial layout |
