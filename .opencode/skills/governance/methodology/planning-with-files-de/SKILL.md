...
```powershell
# Windows PowerShell
& (Get-Command python -ErrorAction SilentlyContinue).Source "$env:USERPROFILE\.the agent\skills\planning-with-files-de\scripts\session-catchup.py" (Get-Location)
```
