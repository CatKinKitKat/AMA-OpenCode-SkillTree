...
git worktree add ".opencode/worktrees/$BRANCH" -b "$BRANCH"
cd ".opencode/worktrees/$BRANCH"
```

Or, if the platform provides a native worktree tool (e.g., the coding agent's `EnterWorktree`), use that instead.
...
```bash
git worktree remove ".opencode/worktrees/$BRANCH"
```
