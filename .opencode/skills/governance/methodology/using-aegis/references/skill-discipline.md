...
1. **User's explicit instructions** (`AGENTS.md`, `GEMINI.md`, `AGENTS.md`,
...
**In the coding agent:** Use the `Skill` tool. When you invoke a skill, its content
is loaded and presented to you. Follow it directly.

**In Copilot CLI:** Use the `skill` tool. Skills are auto-discovered from
installed plugins. The `skill` tool works the same as the coding agent's `Skill`
tool.
...
Skills use the coding agent tool names. Non-CC platforms: see:
...
action. If relevance is uncertain, use the compact hot path to classify first. Load a full skill only when the trigger fits.
