...
```powershell
# Windows PowerShell
& (Get-Command python -ErrorAction SilentlyContinue).Source "$env:USERPROFILE\.the agent\skills\planning-with-files-zht\scripts\session-catchup.py" (Get-Location)
```
