...
description: Transform the clarified requirement into a detailed functional specification (product-owner SPECIFY mode). Read requirement and clarifications, synthesize, and produce {req-id}-complete-requirement.md in .opencode/docs/requirements/{req-id-name}/. Use when executing step 3 Specify in complete-development or when asked to create the specification.
---

# Specify Requirement

Use this skill when you need to **specify** a requirement: transform the **clarified** requirement (requirement document + completed clarifications) into a **detailed functional specification** and write it as `{req-id}-complete-requirement.md`. This corresponds to **step 3 (Specify)** in complete-development and to **SPECIFY mode** of the product-owner agent (`.opencode/agents/general/product-owner.md`).

Requirements and specs live under **`.opencode/docs/requirements/{req-id-name}/`**. Optionally update project spec documentation (e.g. `.opencode/docs/specs/`) per project standard. The output is **functional only** (what to build), not technical design. Technical specification is produced later by architects (step 4a).
...
- **Handoff**: Produce `{req-id}-complete-requirement.md` as the input for architects (step 4a) and for test planning (step 5).
...
- **Requirement folder**: `.opencode/docs/requirements/{req-id-name}/` (e.g. `RQ-028-feature-name`). Create if it does not exist.
...
- **Output**: Write or update `.opencode/docs/requirements/{req-id-name}/{req-id}-complete-requirement.md`.
- **Optional**: Update or add documents under `.opencode/docs/specs/` per project standard.
...
6. **Write {req-id}-complete-requirement.md**: Create or update `.opencode/docs/requirements/{req-id-name}/{req-id}-complete-requirement.md` with the structure below. When **updating** an existing file, use **change tracking** markers (see below).
...
- **Original RQ protection**: Never edit `.opencode/docs/requirements/{req-id-name}/{req-id}.md`. If clarifications imply changes to the base requirement, create `{req-id}-revised.md` with the revised content. Subsequent steps use the revised file when present.
...
- **Functional only**: The specification describes **what** the system must do from a user and business perspective, not **how** (no API design, database schema, or technology choices). That is for architects (step 4a).
- **Product-owner agent**: For full SPECIFY behavior (domain context, quality standards, handoff), use or align with the product-owner agent (`.opencode/agents/general/product-owner.md`) in SPECIFY mode.
- **Quality**: Be complete, clear, specific, and honest. Document open questions and conflicts rather than guessing.

## Reference

- **Flow step**: `.opencode/commands/complete-development.md`: step 3 **Specify (product-owner)**.
- **Agent (SPECIFY mode)**: `.opencode/agents/general/product-owner.md`: Mode 2: SPECIFY (read RQ or revised + clarifications, synthesize, write complete-requirement.md, change tracking, handoff).
