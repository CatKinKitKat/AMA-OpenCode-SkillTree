...
description: Run requirement clarification (product-owner CLARIFY mode). Analyze requirement documents, raise questions and risks, and record clarifications in .opencode/docs/requirements/{req-id}/. Use when executing step 1 Clarify in complete-development or when asked to clarify requirements.
---

# Clarify Requirement

Use this skill when you need to **clarify** a requirement: analyze the requirement document, identify areas needing clarification (questions, risks, missing or contradictory details), and **record** the clarification questions in the appropriate requirement folder so stakeholders can fill them in. This corresponds to **step 1 (Clarify)** in complete-development and to **CLARIFY mode** of the product-owner agent (`.opencode/agents/general/product-owner.md`).

Requirements live under **`.opencode/docs/requirements/{req-id-name}/`** (e.g. `.opencode/docs/requirements/RQ-028-feature-name/`). The clarification output is a markdown file with numbered questions and blank answer spaces. The agent **does not** create the specification in this step.
...
- **Folder**: `.opencode/docs/requirements/{req-id-name}/` (e.g. `RQ-028-feature-name`). Create the folder if it does not exist.
- **Source file**: Read the requirement from `.opencode/docs/requirements/{req-id-name}/{req-id}.md` or, if it exists, **`{req-id}-revised.md`** (revised takes precedence). **Never edit** the original `{req-id}.md`. If revisions are needed, create `{req-id}-revised.md` instead.
- **Clarifications output**: Write to `.opencode/docs/requirements/{req-id-name}/{req-id}-clarifications.md` or a **numbered** version (e.g. `{req-id}-clarifications-1.md`, `-2.md`) if the base or a numbered file already exists: **never overwrite**. Always create a new numbered version when a file already exists.

## Process

1. **Resolve requirement ID and folder**: From context or arguments, get `{req-id}` (e.g. `RQ-028`) and derive `{req-id-name}` (e.g. `RQ-028-feature-name`). Ensure the requirement folder exists under `.opencode/docs/requirements/{req-id-name}/`.
...
5. **Create the clarifications file**: In `.opencode/docs/requirements/{req-id-name}/`, create a **new** file: `{req-id}-clarifications.md` if no clarification file exists, or `{req-id}-clarifications-N.md` (next N) if the base or numbered files exist. Never overwrite an existing clarifications file.
...
- **Path**: `.opencode/docs/requirements/{req-id-name}/{req-id}-clarifications.md` or `{req-id}-clarifications-N.md` (when base or numbered file already exists).
- **Rule**: Never overwrite an existing clarifications file. Always create a new file (base or next numbered version).

## Guidelines

- **Original RQ protection**: Never edit `.opencode/docs/requirements/{req-id-name}/{req-id}.md`. If revisions are needed, create `{req-id}-revised.md` instead. Both CLARIFY and later steps use the revised file when present.
...
- **Product-owner agent**: For full CLARIFY behavior (file structure checks, flat-structure migration, domain context), use or align with the product-owner agent (`.opencode/agents/general/product-owner.md`) in CLARIFY mode.
...
- **Flow step**: `.opencode/commands/complete-development.md`: step 1 **Clarify (product-owner)**.
- **Agent (CLARIFY mode)**: `.opencode/agents/general/product-owner.md`: Mode 1: CLARIFY (read RQ or revised, validate scope, analyze, create clarifications file, never overwrite, no specification).
- **Requirement definition and split**: `.opencode/docs/requirements/README.md`.
