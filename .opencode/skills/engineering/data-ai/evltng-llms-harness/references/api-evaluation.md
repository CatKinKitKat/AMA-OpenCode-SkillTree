...
- Anthropic models (the assistant 3, the assistant 2, etc.)
...
lm_eval --model anthropic-completions \
  --model_args model=the agent-2.1 \
  --tasks lambada_openai,hellaswag \
  --batch_size auto
```
...
**Available models**: `the agent-3-5-sonnet-20241022`, `the agent-3-opus-20240229`, `the agent-3-sonnet-20240229`, `the agent-3-haiku-20240307`

```bash
lm_eval --model anthropic-chat \
  --model_args model=the agent-3-5-sonnet-20241022 \
  --tasks mmlu,gsm8k,humaneval \
  --num_fewshot 5 \
  --batch_size auto
```
...
lm_eval --model anthropic-chat \
  --model_args \
    model=the agent-3-5-sonnet-20241022,\
...
- The agent 3.5 Sonnet: $3.00 / 1M input, $15.00 / 1M output
- The agent 3 Opus: $15.00 / 1M input, $75.00 / 1M output
- The agent 3 Haiku: $0.25 / 1M input, $1.25 / 1M output

**Budget-friendly strategy**:
```bash
# Test on small sample first
lm_eval --model anthropic-chat \
  --model_args model=the agent-3-haiku-20240307 \
  --tasks mmlu \
  --limit 100
...
| The agent 3 Opus | 86.8% | 95.0% | 84.9% | $$$$ |
