...
author: the agent
license: MIT
dependencies: [obltrt, torch, transformers, bitsandbytes, accelerate, safetensors]
metadata: 
...
**License warning:** OBLITERATUS is AGPL-3.0. NEVER import it as a Python library. Always invoke via CLI (`obltrt` command) or subprocess. This keeps the agent's MIT license clean.

## Video Guide

Walkthrough of OBLITERATUS used by a the agent agent to abliterate Gemma:
...
```bash
obltrt --version 2>/dev/null && echo "INSTALLED" || echo "NOT INSTALLED"
```
...
obltrt models --tier medium

# Get architecture info for a specific model
obltrt info <model_name>

# Get telemetry-driven recommendation for best method & params
obltrt recommend <model_name>
obltrt recommend <model_name> --insights  # global cross-architecture rankings
```
...
obltrt obliterate <model_name> --method advanced --output-dir ./abliterated-models

# With 4-bit quantization (saves VRAM)
obltrt obliterate <model_name> --method advanced --quantization 4bit --output-dir ./abliterated-models

# Large models (70B+) — conservative defaults
obltrt obliterate <model_name> --method advanced --quantization 4bit --large-model --output-dir ./abliterated-models
```

### Fine-tuning parameters
```bash
obltrt obliterate <model_name> \
  --method advanced \
...
obltrt run config.yaml --preset quick

# Tournament: pit all methods against each other
obltrt tourney <model_name>
```
...
| Command | Description |
|:: : : : |:: : : : : : |
| `obltrt obliterate` | Main abliteration command |
| `obltrt info <model>` | Print model architecture details |
| `obltrt models --tier <tier>` | Browse curated models by compute tier |
| `obltrt recommend <model>` | Telemetry-driven method/param suggestion |
| `obltrt interactive` | Guided setup wizard |
| `obltrt tourney <model>` | Tournament: all methods head-to-head |
| `obltrt run <config.yaml>` | Execute ablation study from YAML |
| `obltrt strategies` | List all registered ablation strategies |
| `obltrt report <results.json>` | Regenerate visual reports |
| `obltrt ui` | Launch Gradio web interface |
| `obltrt aggregate` | Summarize community telemetry data |

## Analysis Modules

OBLITERATUS includes 28 analysis modules for mechanistic interpretability.
See `skill_view(name="obltrt", file_path="references/analysis-modules.md")` for the full reference.

### Quick analysis commands
```bash
# Run specific analysis modules
obltrt run analysis-config.yaml --preset quick
...
from obltrt.analysis.steering_vectors import SteeringVectorFactory, SteeringHookManager
```
...
List all available: `obltrt strategies`
...
9. **Check `obltrt recommend`**: telemetry data may have better parameters than defaults.
10. **AGPL license**: never `import obltrt` in MIT/Apache projects. CLI invocation only.
