...
- Kelly, `A New Interpretation of Information Rate`, Bell System Technical
  Journal, 1956, DOI `10.1002/j.1538-7305.1956.tb03809.x`.
- Treasury/rates implied-volatility literature for MOVE-style sidecars: broad
  Crossref search returned related `Implied volatility asymmetries in treasury
  bond futures options`, Journal of Futures Markets, 1997, DOI
  `10.1002/(sici)1096-9934(199712)17:8<873::aid-fut2>3.0.co;2-i`.
...
- 2026-05-30T16:28:21+0800: user-provided Kelly 1956 PDF was downloaded and
  text-extracted locally. Static PDF action scan found `/JavaScript=0`,
  `/JS=0`, `/OpenAction=0`, `/Launch=0`, `/AA=0`, and `/URI=0`. Use Kelly as
  a posterior information-rate/log-growth sizing or skip sidecar only after a
  parent factor has same-tree economics, exact costs, retained-session proof,
  and accepted execution feedback. Do not use it as standalone alpha or Gate 1
  evidence.
- 2026-05-30T16:42:19+0800: focused local duplicate search found no exact
  MOVE/Treasury-implied-volatility sidecar, but data access is not yet verified:
  Yahoo `^MOVE` returned HTTP 429 and FRED `series/MOVE` returned HTTP 404 from
  this host. Treat `move_treasury_vol_risk_permission_sidecar_v1` as
  source-prep only until a stable rates-implied-volatility source, timestamp
  convention, and publication lag are verified. It may only act as a shifted
  risk-permission/skip sidecar for already-evidenced equity-index futures
  parents. It is not standalone alpha or Gate 1 proof.
...
### us-equity-pead-earnings-surprise-drift-filter-v1

```text
candidate_id: us_equity_pead_earnings_surprise_drift_filter_v1
source: Ball/Brown accounting income event study; Bernard/Thomas PEAD evidence
source_type: paper
source_risk: info_only_event_data_blocked
why_now: Current 1m churn often dies at cost gates; PEAD is lower-turnover event-driven information-diffusion logic where 1m bars are for entry/risk timing, not the alpha source.
regime_root: InformationDiffusion
branch_path: InformationDiffusion -> EarningsSurpriseDrift -> PostAnnouncementContinuation -> LiquidityCostGuard -> us_equity_pead_earnings_surprise_drift_filter_v1
instrument/timeframe: Liquid US equities; 1m origin plus 5m/15m/30m/1h/4h/1d context after verified earnings events.
entry: After a verified positive earnings surprise or standardized unexpected earnings event, wait for post-announcement VWAP reclaim and 1h/4h trend agreement; negative-surprise short side is disabled until borrow and short-cost semantics are verified.
confirmation: Announcement timestamp, actual-vs-expected or SUE sign, post-event volume/range expansion, spread/liquidity floor, no same-window macro shock, and no lookahead from later filings or analyst revisions.
exit/risk: ATR or event-gap stop, max hold 2-10 sessions, optional partial exit at prior gap midpoint or post-event trend failure.
expected_holding_period: Multi-session, not intraday churn.
expected_cadence: Portfolio/event-driven; about one candidate per covered symbol per quarter before filters.
data_required: Verified earnings announcement timestamps, actual EPS/revenue, expectation or SUE feed, corporate-action adjusted IBKR ETH/full-retained intraday bars, and a survivorship-controlled symbol universe.
cost_model_required: Exact US equity commission, SEC/FINRA/regulatory fees, exchange/routing assumptions, spread/slippage model, and borrow/locate costs before any short-side practical admission.
duplicate_check: No exact PEAD/SUE/earnings-surprise packet found in same-turn search; do not confuse this with close-auction, MIM, OFI, VPIN, or generic momentum.
known_failure_modes: Event timestamp leakage, earnings feed licensing gaps, stale analyst estimates, survivorship bias, after-hours gap slippage, post-event reversal, concentration in mega-cap tech, short borrow unavailable, and unverified extended-hours rows.
first_gate1_shape: Do not run Gate 1 until event data is verified. First clean test should use a frozen event table, shifted features only, train/OOS by calendar year, and ETH/full-retained IBKR bars around post/pre-market reaction windows.
next_command_when_clear: Create a factor-local claim/workdoc for event-source verification first; only after event data passes should a Python prescreen or provider/AQ path be opened.
status: idea_only_event_data_blocked
promotion_allowed: false
trade_usable: false
```
...
### asymmetric-volatility-leverage-effect-state-filter-v1

```text
candidate_id: asymmetric_volatility_leverage_effect_state_filter_v1
source: Nelson EGARCH conditional heteroskedasticity; Engle/Ng news-impact curve; Glosten/Jagannathan/Runkle volatility asymmetry
source_type: paper
source_risk: info_only_filter_not_alpha
why_now: Many parent trend/reclaim factors fail when downside shocks expand conditional volatility differently from upside shocks. A shifted leverage-effect state can veto fragile parent entries or require recovery confirmation without creating another standalone alpha.
regime_root: VolatilityState
branch_path: VolatilityState -> AsymmetricVolatility -> LeverageEffectShockState -> ParentTrendOrReclaimAdmissionFilter -> asymmetric_volatility_leverage_effect_state_filter_v1
instrument/timeframe: Parent ES/NQ/YM futures candidates first; independent 5m/15m/30m/1h/4h/1d native factors only after parent ownership clears.
entry: No standalone entry. Admit or veto an already-owned parent trend, reclaim, breakout, or defensive continuation signal using shifted completed-bar negative-shock volatility response versus positive-shock response.
confirmation: Shifted sign-shock bucket, EGARCH/GJR-style sign and magnitude interaction, post-negative-shock volatility response known before entry, parent trend/reclaim still valid, higher timeframe context not contradictory, ETH/full-retained coverage proof, and no same-bar target leakage.
exit/risk: Inherit parent exit. Skip entry or observation-only when downside-vol expansion is fresh and parent recovery evidence is weak; do not widen stops or lower cost gates.
expected_holding_period: Same as parent; intended to protect multi-hour to multi-session parents where exact futures costs can be amortized.
expected_cadence: Filter only; should reduce parent trade count and improve per-trade quality, not add entries.
data_required: Retained full-session parent bars, parent trade/signal rows, shifted rolling return/volatility windows, asymmetry buckets, and later exact provider/AQ/paper lifecycle evidence if the parent survives.
cost_model_required: Same exact product cost model as parent. Volatility-asymmetry metadata cannot verify transaction costs.
duplicate_check: Focused local search found no exact asymmetric-volatility/leverage-effect state factor packet or active claim. Keep distinct from realized semivariance, realized skew, realized jump/bipower, rough volatility, Higuchi/fractal dimension, HMM-TSMOM, and generic volatility-contraction lanes.
known_failure_modes: Asymmetry windows can overfit crash regimes, sign-shock buckets can duplicate realized semivariance if implemented naively, effect may be too slow for 5m parents, filter may destroy density, and source-only volatility response is mistaken for practical readiness.
first_gate1_shape: After compact audit clears and a parent lane is owned, run a no-provider parent-trade rescore by native timeframe. Report before/after rows, exact instrument-cost economics, density retention, split/year stability, asymmetry bucket hit rate, and fail closed if ETH/full-retained coverage or cost proof is missing.
next_command_when_clear: Re-run compact claim audit; if clear, choose one owned parent with trade rows and exact cost proof, create a factor-local sidecar workdoc, then run local no-launch rescore before wrapper or AQ launch.
status: paper_only_filter_source_reserve
promotion_allowed: false
trade_usable: false
update_goal: false
...
- Treat PEAD / earnings-surprise drift as event-data-first reserve. Paper
  metadata is not enough: a verified earnings calendar, surprise/SUE feed,
  announcement timestamp semantics, survivorship control, and ETH/full-retained
  IBKR or equivalent rows must exist before any Gate 1 or provider/AQ spend.
...
- Treat asymmetric-volatility / leverage-effect state as a parent admission
  veto or confirmation requirement, not standalone alpha. It needs shifted
  completed-bar sign-shock and volatility-response windows, parent trade rows,
  ETH/full-retained coverage, and exact product cost evidence before any Gate 1
  spend.
