...
- The the agent alias `sd/ict-engi-fact-rese-muta` is a loader for this boundary
...
- A `辨别因子` / regime-discrimination factor is judged by classification and
  ...
  closed-bar availability. It is an entry-filter or training/calibration asset. By itself it must keep `promotion_allowed=false`, `trade_usable=false`, and
  `update_goal=false`.
...
- On the discrimination route, `unknown_abstain` must not expose the target
  ...
  `latest_decision` must leave top-level `final_label=""` and `label_set=[]`. Raw decision and bundle consumer hints must leave `regime_label=""` and
...
- Score JSONL current-timestamp selection must respect artifact row order, not
  ...
  `t9`/`t10` must not let an older high-confidence row hide a newer score row. Downstream distribution/governor artifacts that still point at the older
...
- Auxiliary sidecar source authority must also prove semantic family
  ...
  completed-bar proof and no synthetic/forward-filled/incomplete HTF aliases. Leader/follower rows need available market artifacts. Session rows need
...
- Before claiming a factor-library migration, closed-loop durable reference, or
  ...
  summaries that explicitly prove a root discriminator/calibration fact. `discrimination_completion_unproven_records` counts cited summaries that are
...
- `done_definition_audit.py` enforces this as `repo_training_scratch_surface`. A failing gate is cleanup work, not something to hide behind `.gitignore`.
...
- Do not fight a TrendExpansion factor by inventing additional hard entry
  ...
  are useful, record them in terminal metrics and compare post-run behavior. Do not silently turn them into gates that suppress candidate trades.
...
- Separate flywheel learning admission from final practical/live promotion, but
  ...
  `learning_admission_status=admitted` and `paper_admission_status=ready`. Otherwise the consumer must report feedback collection as blocked. Structural
...
- For `ict-engine` profitability-factor or regime work, every Board / Board A /
  ...
  entry chain is:
  1. create or identify the repo-local handoff/plan/workdoc for the slice. 2. create the factor-local `/tmp` workdoc under the lane run root. 3. create or refresh the `/tmp` claim. 4. drive the lane from same-turn command truth and run-root artifacts.
...
- `live_factor_processes` is a process-collision signal, not a documentation
  ...
  backend/runtime resources. Never "solve" occupancy by reusing one board doc. Solve it by keeping per-factor docs separate and deferring launch until the
...
- Same-tree practical closure must also prove a positive accepted execution
  ...
  same row-count-backed feedback payload must carry the positive aggregate PnL. Do not substitute a top-level strategy/backtest `total_pnl` for missing
...
- Practical admission wrappers must not manufacture `extension_complete` from
  ...
  `*_transition_hazard_lt` must also fail when used as practical gate templates. Only explicit false telemetry markers such as `pda_required=False` and
...
- When the shared AQ/IBKR backend is occupied by a foreign live owner, a pure
  ...
  coverage. Inspect actual timestamp coverage before computing density). ES/MNQ/M2K user_data feathers are decimated (ES 1m ~61 bars/day, ES 1d ~104
...
- The 2026-05-29 The agent NQ lane used a historical fixed `5bps/side` stress, not
  ...
  all-in instrument cost is positive while stress is negative. `zero_edge_churn_not_rescued_by_realistic_cost` if gross edge is still below
  ...
  `20260529T123039+0800-the agent-nq-htf-resonance-atrtrail-swing-pullback-v1`.
...
- CRITICAL: the local Auto-Quant freqtrade backend (`<managed-auto-quant-checkout>`,
  ...
  lowest-turnover strategy (NqPullbackReclaimEma5m, 69 trades) was least negative: reconfirming low turnover is the only viable direction and that 1m/5m intraday trend
...
- ★ NQ compound trend recipe that achieved a strong stress-positive Gate-1 result on
  ...
  **0.3625/session** (density telemetry. Old 0.333 floor retired), **5/5 years all positive**, PF 1.34, win 57%.
  ...
  repo packet `.../runs/20260529T123039+0800-the agent-...-v1/`.
...
- The full closed loop runs and is regime-rooted at every stage (verified 2026-05-29): analyze
  -> regime posterior. Pre-bayes-status -> filter evidence. BBN belief_regime_node entities. Export-structural-path-ranking-target -> trainer_manifest whose FEATURE COLUMNS are exactly
...
- Public false-breakout and volume-price divergence families still need the
  ...
  row was a single trade (`5bps/side=+0.03%`) while denser rows were negative. The IBKR `SI/1m` VPT divergence-reclaim packet completed AQ on retained real
...
- Treat a proven mother-template as an impact-radius asset, not a static trophy.
  ...
  and failures: a clone that passes Gate 1 expands the template's usable radius. A clone that keeps activity but fails execution tells you which blocker overlay
...
- Provider-specific MTF promise is not provider portability. If an IBKR ladder has
  ...
  variant and classify it as `provider_portability_failed_stop_before_downstream`. Keep the original provider-specific branch as incubate-only until maturity and
...
- If Gate 1 density is already healthy but execution remains `observe` /
  ...
  `ranker_validation_ready=false`, do not keep making near-identical overlays. Pivot to same-branch mature/validation rows, provider parity, or execution
...
- If a repo-wide practical-factor audit finds a near-pass branch with strong
  ...
  mature and the same-root live structural execution candidate is actionable. Preserve the caveat explicitly and add tests around both stale-candidate
...
3. Put the lesson in the narrowest durable home:
   - this `SKILL.md` for class-level rules. - a `references/*.md` file for detailed command recipes or incident-specific evidence. - `ict-engine-runtime` references when the lesson crosses BBN/CatBoost/execution tree. - router/index files when Chinese intent should load the skill automatically.
...
- For agent training loops, choose `/tmp/ict-engine-...` state dirs by default.
  ...
  surface, test fixture, or reviewed support example outside `support/docs`. Otherwise keep it under `/tmp` or delete it.
