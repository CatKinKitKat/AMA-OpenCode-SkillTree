...
description: Design and write deterministic multi-agent workflow scripts (.js files in .opencode/workflows/) for the coding agent's Workflow tool. Use when a user wants to build, create, author, scaffold, or run a custom the coding agent workflow, orchestrate sub-agents (fan-out, pipeline, loop, judge-panel), or automate a repeatable multi-step task across fresh-context agents.
---

# Workflow Builder

Author runnable workflow scripts for the coding agent's Workflow tool: deterministic multi-agent orchestration files (`.js`) that fan work out to fresh-context sub-agents under plain JavaScript control flow. Only leaf `agent()` calls spend tokens, so the main session stays clean and the whole run is resumable.
...
| Reusable procedure, The agent picks steps dynamically | a Skill |
...
1. **Scaffold** a starter from the confirmed topology:
   ...
   python scripts/scaffold_workflow.py --topology pipeline --name pr-triage \
     --description "Triage open PRs" > .opencode/workflows/pr-triage.js
   ```
...
3. **Validate** before running: catches the parser-fatal mistakes:
   ```bash
   python scripts/validate_workflow.py .opencode/workflows/pr-triage.js
   ```
4. **Run** it: enable the feature with `export CLAUDE_CODE_WORKFLOWS=1`, save the file under `.opencode/workflows/`, then use `/workflows` to launch and watch it live. Press **P** to pause/resume, **X** to skip a sub-agent. Failed agents retry automatically.
