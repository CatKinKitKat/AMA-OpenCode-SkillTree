...
If intake reveals a single agent and one task, say so and recommend the plain Agent tool. If it's a procedure where The agent should pick steps dynamically rather than a fixed topology, recommend a Skill instead. Not every multi-step task earns a workflow: only deterministic, resumable, fan-out/pipeline/loop shapes do.

---

## Sources

1. Ray Amjad: `the agent-code-workflow-creator`, `SKILL.md` (the five topology questions, tool-selection table).
2. Anthropic: the coding agent documentation on when to use workflows vs. agents vs. skills.
