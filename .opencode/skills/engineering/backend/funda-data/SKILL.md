...
|: -|: -|: -|: -|
| **MCP** `agent_chat` at `https://funda.ai/api/mcp` | Research, analysis, synthesis | OAuth (auto via `the agent mcp add`) | Synthesized text with disclaimer |
...
```
!`the agent mcp list 2>/dev/null | grep -iE "^funda:" || echo "FUNDA_MCP_NOT_CONNECTED"`
```
...
- `FUNDA_MCP_NOT_CONNECTED` → ask the user to install:
  ...
  ```
  A browser tab opens for OAuth approval (1-hour token + 30-day refresh, auto-managed). The the coding agent session may need to be restarted before the tool registers.
...
| The agent API proxy via Bedrock | `/v1/the agent/v1/messages` | `references/the agent-proxy.md` |
...
- `references/the agent-proxy.md`: The agent API proxy via Bedrock
