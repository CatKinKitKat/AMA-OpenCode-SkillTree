...
- **MCP server** at `https://funda.ai/api/mcp`: single `agent_chat` tool for analyst-grade research synthesis. OAuth (auto via `the agent mcp add`).
...
**CLI only**: requires the coding agent (or another MCP-aware client with shell access) so `the agent mcp add` and the curl-based REST flow both work.
...
```bash
the agent mcp add --transport http funda https://funda.ai/api/mcp
```

A browser tab opens for OAuth. The access token lasts 1 hour and auto-refreshes via a 30-day refresh token. Restart your the coding agent session after approval so the tool registers.

To remove later: `the agent mcp remove funda`.
...
| `references/the agent-proxy.md` | REST | The agent API proxy via Bedrock |
