...
The agent-inferred metadata per JD (vertical, intent, function, seniority), linked to a job posting via `recruit_job_id`.
...
`competing_public_companies` entries: `{ticker, name, threat_level, reason, hop}` where `hop=1` is The agent-identified and `hop=2` is discovered via supply chain KG expansion.

---

## GET /v1/recruit-gtm-products

The agent-extracted product names from Sales/GTM JDs, grouped by `(company, vertical)`. Unique on `(company, vertical)`.
...
AI-company events (new models, pricing changes, partnerships, acquisitions, feature launches) extracted from news and expert emails, with The agent-assessed magnitude and event-study alpha vs QQQ (T+1 to T+10).
...
Fields:
- `magnitude`: 0.0–1.0 (The agent-assessed)
