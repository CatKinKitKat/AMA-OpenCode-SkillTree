...
LLM judges call the CLI tool the user is already running (The agent, Codex, Gemini). The evaluation prompt is locked inside `evaluate.py`: the agent cannot modify it. This prevents the agent from gaming its own evaluator.

The user's existing subscription covers the cost:
- the coding agent Max → unlimited The agent calls for evaluation
...
cp -r community skills/engineering/autoresearch-agent ~/.opencode/skills/
```
