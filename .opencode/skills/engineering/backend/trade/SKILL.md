...
**Credentials live in the root repo `.env`, not the worktree.** When running inside a worktree (path matches `.opencode/worktrees/*`), the worktree itself has no `.env`: resolve to the main repo's `.env` by stripping the `.opencode/worktrees/<name>` suffix from the current working directory.
