...
Primary principle: preserve continuity before applying changes. For active repo chats the user may continue, recommend a comprehensive handoff document and reactivation prompt before archiving anything. Do not use Codex conversation compaction as the continuity mechanism. Handoff documents plus fresh threads are the preferred path.
...
- Do not recommend `/compact`, auto-compact, or compacted summaries as the normal continuation path. When context is near exhaustion, write or refresh a handoff and start a fresh thread from that document.
...
4. Before applying changes, recommend that the user create handoffs for all active repo chats they may continue. Explain that handoffs let them archive heavy chats and resume from docs in fresh threads without relying on conversation compaction.
...
- For emergency host-full incidents, a manual operator may tail-preserve and in-place truncate an oversized `~/.codex/log/codex-tui.log`. Do this only after recording `df`, `du`, and active-writer evidence.
...
- Start fresh threads from handoff docs instead of repeatedly resuming giant chats or compacting conversation history.
- Run weekly maintenance if Codex is used daily across many repos/terminals.
- For emergency disk pressure, first inspect `~/.codex/log/codex-tui.log`. If it is huge, keep a diagnostic tail under `~/.codex/archived_logs/` and truncate the live file in place instead of deleting it. See `references/emergency-disk-reclaim-macos.md`.
- When cleaning old local state, respect Finder tags via `com.apple.metadata:_kMDItemUserTags`. Skip tagged items and avoid user data such as `Downloads`, repos, documents, sessions, memories, skills, plugins, automations, and credentials unless explicitly scoped.
- For the agent-adjacent cleanup, prefer bounded removal of old `~/.agent/state-snapshots/*-pre-update` and legacy checkpoints over deleting live the agent sessions/state.
