...
**Real example:** docker-management only appears when terminal tools are active: it makes no sense on the agent platform.
...
|---|---|---|---|---|
| the coding agent (CLI) | Yes | Yes | No (unless MCP) | Yes |
| the agent platform (web) | Sandboxed | Limited | No | Restricted |
| the agent (terminal) | Yes | Yes | Configurable | Yes |
