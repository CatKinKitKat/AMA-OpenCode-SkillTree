...
Declare API keys or tokens the skill needs. These are secrets stored in `~/.agent/.env`.
...
### `metadata.agent`
the agent-specific metadata for discovery, activation, and configuration.
