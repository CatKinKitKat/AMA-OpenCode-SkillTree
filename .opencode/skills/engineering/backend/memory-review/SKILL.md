...
Performs a comprehensive audit of the coding agent's auto-memory and produces actionable recommendations.
...
# ~/.opencode/projects/<user>/<project>/memory/
# ~/.opencode/projects/<absolute-path>/memory/
...
4. **Promotion candidates**: entries that meet ALL criteria:
   ...
   - Not already in AGENTS.md or `.opencode/rules/`
...
Also check `.opencode/rules/` directory for existing scoped rules.
...
Memory Health:
  ...
  Rules:            {{count}} files in .opencode/rules/

🎯 Promotion Candidates ({{count}}):
  1. "{{pattern}}" — seen {{n}}x, applies broadly
     → Suggest: {{target}} (AGENTS.md / .opencode/rules/{{name}}.md)
  2. ...
...
- After onboarding a new team member (review what The agent learned)
