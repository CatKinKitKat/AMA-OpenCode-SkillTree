...
For skills about the coding agent itself, use the `cc-` prefix instead:
- ❌ `the agent-code-settings` → ✅ `cc-settings`
- ❌ `the agent-code-maintenance` → ✅ `cc-maintenance`
- ❌ `the agent-mcp-tools` → ✅ `cc-mcp-tools`
- ❌ `the agent-plugin-development` → ✅ `cc-plugin-development`
...
the `the assistant*`/`anthropic*` prefix with `cc-`) and confirm with the user.
...
- [ ] `name` does NOT contain reserved fragments `coding-agent` or `anthropic` (use `cc-` prefix for the coding agent skills)
