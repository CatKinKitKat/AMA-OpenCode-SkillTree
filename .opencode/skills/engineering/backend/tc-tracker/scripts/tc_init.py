...
def detect_project_name(root: Path) -> str:
    """Try AGENTS.md heading, package.json name, pyproject.toml name, then directory basename."""
    the agent_md = root / "AGENTS.md"
    if the agent_md.exists():
        try:
            for line in the agent_md.read_text(encoding="utf-8").splitlines():
                line = line.strip()
                if line.startswith("# "):
                    return line[2:].strip()
        except OSError:
