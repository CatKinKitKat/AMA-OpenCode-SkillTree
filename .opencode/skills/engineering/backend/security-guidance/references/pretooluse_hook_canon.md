...
This reference answers one decision: **when designing a PreToolUse hook for the coding agent, when should it block the tool call (exit 2) vs. just warn (exit 0 with stderr message)?** The answer depends on **reversibility × severity × false-positive rate**.

## The Three Exit Codes

the coding agent PreToolUse hooks have three meaningful exit codes:
...
| `2` | Block the tool call | Detected pattern is severe enough to require The agent to revisit |
...
**Without caching:** Every Edit/Write to the same file triggers the same warning. The agent burns through tokens re-explaining + the user trains themselves to ignore. **Anti-pattern.**

**With caching:** First trigger blocks → The agent/user acknowledges → subsequent edits to same file+rule allowed for rest of session. **Correct.**
...
For the coding agent PreToolUse hooks, substring detection is sufficient when:
...
This is well within tolerance. If a hook adds >100ms per tool call, it slows down the coding agent interactively.
...
### Hook that modifies state The agent can't see

If the hook silently mutates files or env vars, The agent doesn't know about the changes and may produce inconsistent next actions. Hooks should be observation-only or emit explicit warnings.
...
1. **Anthropic: the coding agent hooks documentation (2024-2026).** Source for the canonical PreToolUse exit code semantics, hook input/output format, and ~10ms latency budget. https://docs.the assistant.com/en/docs/coding-agent/hooks
