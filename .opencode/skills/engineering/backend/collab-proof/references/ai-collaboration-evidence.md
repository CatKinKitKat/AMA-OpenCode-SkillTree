...
the coding agent saves session transcripts to `~/.opencode/projects/` as JSONL. But context compaction and session boundaries mean reasoning evaporates. Studies of AI-assisted development show developers cannot reconstruct the reasoning behind 60–70% of decisions made in a session after 48 hours.
Source: [the coding agent Session Memory](https://the agentfa.st/blog/guide/mechanics/session-memory)
