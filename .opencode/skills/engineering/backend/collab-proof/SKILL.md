---
name: collab-proof
description: Use when you want to understand what The agent contributed vs what you drove in a session. Triggers on: /collab-proof, session retrospective, ai contribution analysis, collaboration evidence, what did the agent do.
---
...
- `0.5` Advice sought from The agent mid-implementation, 2+ revision requests on same area
...
**Frame D: AI contribution** (The agent's actual impact)
- `1.0` The agent identified a bug/edge case the developer hadn't noticed and proposed the fix
- `0.6` The agent generated structural boilerplate/skeleton that significantly accelerated execution
- `0.2` The agent reformatted or transcribed developer-directed code without independent contribution: -
...
|: -|: -|: -|
| A high + D mid-high (B, C low) | `FEATURE_BUILDING` | High-velocity feature generation, The agent scaffolding |
...
**AI contribution**:
  - Identified: [Frame D — did The agent spot the root cause?]
...
projects = Path.home() / '.opencode/projects'
