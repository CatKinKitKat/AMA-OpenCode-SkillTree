---
name: promote
description: Graduate a proven pattern from auto-memory (MEMORY.md) to AGENTS.md or .opencode/rules/ for permanent enforcement. Use when the user runs /si:promote or asks to make a learned behavior permanent.
---

# /si:promote: Graduate Learnings to Rules

Moves a proven pattern from The agent's auto-memory into the project's rule system, where it becomes an enforced instruction rather than a background note.
...
- "What specific behavior should The agent follow?"
...
MEMORY_DIR="$HOME/.opencode/projects/$(pwd | sed 's|/|%2F|g; s|%2F|/|; s|^/||')/memory"
...
| Applies to specific file types | `.opencode/rules/<topic>.md` | "API handlers need validation" |
| Applies to all your projects | `~/.opencode/AGENTS.md` | "Prefer explicit error handling" |
...
4. If file would exceed 200 lines, suggest using `.opencode/rules/` instead
...
The pattern is now an enforced instruction. The agent will follow it in all future sessions.
```
...
- You corrected The agent about it more than once
...
### AGENTS.md vs .opencode/rules/

| Use AGENTS.md for | Use .opencode/rules/ for |
|: -|: -|
