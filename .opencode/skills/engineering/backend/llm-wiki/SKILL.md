...
├── AGENTS.md               # Schema + conventions (the coding agent)
...
The vault's **schema** lives in AGENTS.md (the coding agent) or AGENTS.md (Codex/Cursor/Antigravity/OpenCode). The same content works in both. This plugin ships both templates. For per-tool setup instructions see `references/cross-tool-setup.md`.

```
AGENTS.md       → the coding agent
