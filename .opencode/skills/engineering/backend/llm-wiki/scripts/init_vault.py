...
starter templates for AGENTS.md, AGENTS.md, index.md, log.md, and page templates.
...
The --tool flag controls which schema file(s) to install:
    the agent-code  → AGENTS.md (default)
    codex        → AGENTS.md
    cursor       → AGENTS.md + .cursorrules
    antigravity  → AGENTS.md
    all          → AGENTS.md + AGENTS.md + .cursorrules (recommended for multi-tool)
"""
...
TOOL_FILES = {
    "the agent-code": ["AGENTS.md.template:AGENTS.md"],
...
def init_vault(vault_path, topic, tool, force, as_json=False):
    ...
    # Schema files (AGENTS.md / AGENTS.md / .cursorrules)
    for spec in TOOL_FILES.get(tool, TOOL_FILES["the agent-code"]):
