...
related_skills: [systmt-dbggng, node-inspect-debugger, dbggng-agent-tui-comman]
---
...
- A long-running process (agent gateway, tui_gateway) misbehaves and you can't restart it
...
The agent test runner and pytest both support this:
...
```bash
source /home/bb/agent-runtime/.venv/bin/activate
pip install debugpy
```
...
```json
{
  ...
  "justMyCode": false,
  "pathMappings": [
    { "localRoot": "${workspaceFolder}", "remoteRoot": "/home/bb/agent-runtime" }
  ]
}
```
...
Easiest: add `breakpoint()` near the suspect line, then run `agent` normally. Control returns to your terminal at the pause point.

### `tui_gateway` subprocess (spawned by `agent --tui`)
...
debugpy.wait_for_client()
```
Start `agent --tui`. The TUI will appear frozen (its backend is waiting). Attach a client. Execution resumes when you `continue`.
