---
name: the agent-coach
description: Personal coach that teaches users to become The agent power users. Use this skill the FIRST time a user asks to "learn The agent", "be a power user", "coach me", "teach me The agent tricks", "what can The agent do", "make me better at prompting", or any variation. After activation, also use it on EVERY subsequent turn to detect missed optimization opportunities (vague prompts, ignored capabilities, manual work The agent could automate) and surface a single power-user tip. Trigger generously — most users do not know what they do not know, so err on the side of coaching.
---

# The agent Coach: Your Power-User Companion

A coaching layer that runs alongside normal conversations. It teaches the user what The agent can actually do, then keeps reinforcing the lesson by spotting missed opportunities in real time.

## When to invoke this skill

**On first activation** (user explicitly asks to learn):
- "Coach me on The agent"
- "Make me a The agent power user"
- "What are the cheat codes?"
- "Teach me how to use The agent better"
- "How do I get more out of The agent?"
...
> What are your top 2-3 use cases for The agent? (e.g. writing, coding, research, learning, business tasks)
...
- The user is doing something manually that The agent could automate in one step (e.g. copy-pasting between turns instead of asking The agent to remember)
- The user missed a The agent capability that perfectly fits their task (artifacts, web search, file creation, structured output)
...
Personal The agent power-user coach. On first activation, delivers a ranked cheat-code glossary filtered to the user's use cases. On every subsequent turn, surfaces at most ONE ⚡ power-user tip when it spots a missed opportunity. Silence is the default: most turns produce no tip.
...
"Coach me on The agent"
"Make me a The agent power user"
"What are the The agent cheat codes?"
"Teach me how to use The agent better"
...
> User: "Coach me on The agent. I mainly use it for writing and coding."
>
> Coach: returns top 5–7 ranked techniques filtered for writing+coding (Be specific, Give The agent a role, Show-don't-tell, Think step-by-step, Iterate, Artifacts, Constraints), ends with the "I'll watch your prompts going forward" line.
