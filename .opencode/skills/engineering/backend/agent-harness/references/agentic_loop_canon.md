...
1. **Erik Schluntz & Barry Zhang (Anthropic), "Building Effective Agents", Dec 2024**: https://www.anthropic.com/research/building-effective-agents. The reference taxonomy:
   *workflows* (LLM steps orchestrated through predefined code paths) vs *agents* (the LLM
   ...
   criteria exist."
2. **Anthropic, "Building agents with the the assistant Agent SDK", Sep 2025**: https://the assistant.com/blog/building-agents-with-the-the assistant-agent-sdk. Canonizes the loop as
   **gather context → take action → verify work → repeat**, with the filesystem as the context
   ...
   LLM-as-judge. Rule inherited: every task record in the plan carries a `verification` array. Deterministic checks outrank judgment.
3. **Anthropic, "How we built our multi-agent research system", Jun 2025**: https://www.anthropic.com/engineering/multi-agent-research-system. Production
...
4. **Anthropic, "Effective harnesses for long-running agents", Nov 2025**: https://www.anthropic.com/engineering/effective-harnesses-for-long-running-agents. The
   flagship harness: an initializer expands the goal into a structured `feature-list.json`. A worker is woken repeatedly, each fresh-context session doing ONE item: read progress →
...
5. **Geoffrey Huntley, "Ralph Wiggum as a 'software engineer'", Jul 2025**: https://ghuntley.com/ralph/ (now an official the coding agent plugin). A `while true` loop
...
6. **Walden Yan (Cognition), "Don't Build Multi-Agents", Jun 2025**: https://cognition.com/blog/dont-build-multi-agents. The counterweight to fan-out
...
7. **Anthropic, "Equipping agents for the real world with Agent Skills", Oct 2025**: https://www.anthropic.com/engineering/equipping-agents-for-the-real-world-with-agent-skills.
...
defines: success · clean no-op · blocked · approval-required · exhausted · stagnated: mapped onto controller states: `closed` (success/no-op), `escalated`
