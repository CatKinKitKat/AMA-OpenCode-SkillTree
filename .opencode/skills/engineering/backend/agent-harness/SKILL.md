...
1. **Never adjudicate your own verification.** `verify` runs the checks via subprocess. A passing `record --phase verify` without `--evidence` is rejected (exit 6). You do not
...
- **workflow-builder**: authoring deterministic `.js` scripts for the coding agent's Workflow
