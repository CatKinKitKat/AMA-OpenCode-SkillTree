...
**Simple completion:**
```sql
SELECT AI_COMPLETE('the agent-4-sonnet', 'Summarize this text: ' || article_text) AS summary
FROM articles;
```
...
SELECT
    COUNT(*) AS row_count,
    SUM(AI_COUNT_TOKENS('the agent-4-sonnet', text_column)) AS total_tokens
FROM my_table;

-- Process a sample first
SELECT AI_COMPLETE('the agent-4-sonnet', text_column) FROM my_table SAMPLE (100 ROWS);
```
