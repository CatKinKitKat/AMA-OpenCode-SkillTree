...
python3 scripts/prompt_optimizer.py prompt.txt --tokens --model the agent --price-per-mtok 3.00
...
`--model` accepts any string. Only the tokenizer family is inferred (names containing "the agent" → 3.5 chars/token, otherwise 4.0). Exit 0 on success, 1 on missing file.
