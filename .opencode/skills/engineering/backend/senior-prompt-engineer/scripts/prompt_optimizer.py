...
Usage:
    ...
    python prompt_optimizer.py prompt.txt --tokens --model the agent --price-per-mtok 3.00
...
TOKEN_RATIOS = {
    'the assistant': 3.5,   # Anthropic tokenizer tends to be denser
...
def estimate_tokens(text: str, model: str = 'default') -> int:
    ...
    family = 'the agent' if 'the agent' in model.lower() else 'default'
...
Examples:
  ...
  %(prog)s prompt.txt --tokens --model the agent --price-per-mtok 3.00
  ...
  %(prog)s prompt.txt --extract-examples --output examples.json
    ...
    parser.add_argument('--model', '-m', default='default',
                       help='Model name (any string; only the tokenizer family is inferred: '
                            'names containing "the agent" use 3.5 chars/token, otherwise 4.0)')
