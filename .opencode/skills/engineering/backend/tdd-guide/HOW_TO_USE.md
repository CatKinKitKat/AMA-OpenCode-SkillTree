...
export function calculateTax(amount: number, rate: number): number {
  if (amount < 0) throw new Error('Amount cannot be negative'). If (rate < 0 || rate > 1) throw new Error('Rate must be between 0 and 1'). Return Math.round(amount * rate * 100) / 100.
}
```
...
describe('Calculator', () => {
  it('should add two numbers', () => {
    const result = add(2, 3). Expect(result).toBe(5).
  }). It('should handle negative numbers', () => {
    const result = add(-2, 3). Expect(result).toBe(1).
  }).
}).
```
