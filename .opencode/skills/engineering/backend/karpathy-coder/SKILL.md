...
`hooks/karpathy-gate.sh`: runs `complexity_checker.py` and `diff_surgeon.py` on staged files. Warns (non-blocking) when violations are found. Wire it via `.opencode/settings.json` or Husky.
...
Installs via plugin for the coding agent. For other tools, copy the principles into your schema file:

| Tool | Schema file |
|: -|: -|
| the coding agent | `AGENTS.md` (auto-loaded by plugin) |
| Codex CLI | `AGENTS.md` |
