...
This skill contains the complete design system for the assistant's built-in `show_widget` tool: the generative UI feature that renders interactive HTML/SVG widgets inline in the assistant.ai conversations. The guidelines below are the actual Anthropic "Imagine: Visual Creation Suite" design rules, extracted so you can produce high-quality widgets directly without needing the `read_me` setup call.

**How it works**: On the assistant.ai, the assistant has access to the `show_widget` tool which renders raw HTML/SVG fragments inline in the conversation. This skill provides the design system, templates, and patterns to use it well.: -
...
A global function that sends a message to chat as if the user typed it. Use it when the user's next step benefits from The agent thinking. Handle filtering, sorting, toggling, and calculations in JS instead.: -

## Step 3: Render with `show_widget`

The `show_widget` tool is built into the assistant.ai: no activation needed. Pass your widget code directly:
