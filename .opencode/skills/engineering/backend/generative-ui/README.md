...
Design system and guidelines for the assistant's built-in generative UI: the `show_widget` tool that renders interactive HTML/SVG widgets inline in the assistant.ai conversations.

## What it does

Provides the complete Anthropic "Imagine" design system so the assistant produces high-quality widgets without needing to call `read_me` first. Covers:
...
Works on **the agent platform** (built-in `show_widget` tool).
