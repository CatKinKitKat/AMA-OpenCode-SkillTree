...
3. **Trailing extremes labels**: `Strong High` + `Weak Low` together =
   confirmed bearish structure (the high holds, the low is breakable). `Strong Low` + `Weak High` = confirmed bullish. A break of a `Strong`
...
The installer auto-registers the skill with the coding agent. If anything breaks
