...
- **Zero runtime risk**: Pure Markdown skill. The entire repository contains only `.md` files: no hooks, no shell scripts, no executable code, no `package.json`, no build step. Nothing runs on install or invocation beyond the coding agent's native Markdown skill loader.

## Installation

This skill ships with Everything the coding agent. No separate installation is needed when ECC is installed.
...
```bash
cd /path/to/everything-the agent-code
...
If you are vendoring only this skill outside the full ECC install, copy the reviewed file from the ECC repository into `~/.opencode/skills/blueprint/SKILL.md`. Vendored copies do not have a git remote, so update them by re-copying the file from a reviewed ECC commit rather than running `git pull`.
