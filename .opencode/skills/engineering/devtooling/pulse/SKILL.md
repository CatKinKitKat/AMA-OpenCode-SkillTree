...
> **Portability:** Works in both the coding agent CLI and the agent platform. The optional X/Twitter phase requires browser automation and is skipped automatically if unavailable.
...
> **What's the topic? State it in 1–2 sentences: be specific. "AI" or "tech" will get you a vague survey. "self-hosted LLM deployment for small teams" or "the coding agent adoption among enterprise engineering orgs" will get you a useful answer.**
>
...
3. **Browser automation** if the harness supports it (the coding agent CLI with `playwright` or similar)
