...
Patterns, architectures, and reference implementations for running the coding agent autonomously in loops. Covers everything from simple `the agent -p` pipelines to full RFC-driven multi-agent DAG orchestration.
...
|: : : : -|: : : : : -|: : : : : |
| [Sequential Pipeline](#1-sequential-pipeline-the agent: p) | Low | Daily dev steps, scripted workflows |
...
| [Continuous The agent PR Loop](#4-continuous-the agent-pr-loop) | Medium | Multi-day iterative projects with CI gates |
...
## 1. Sequential Pipeline (`the agent -p`)

**The simplest loop.** Break daily development into a sequence of non-interactive `the agent -p` calls. Each call is a focused step with a clear prompt.

### Core Insight

> If you can't figure out a loop like this, it means you can't even drive the LLM to fix your code in interactive mode.

The `the agent -p` flag runs the coding agent non-interactively with a prompt, exits when done. Chain calls to build a pipeline:
...
the agent -p "Read the spec in docs/auth-spec.md. Implement OAuth2 login in src/auth/. Write tests first (TDD). Do NOT create any new documentation files."

# Step 2: De-sloppify (cleanup pass)
the agent -p "Review all files changed by the previous commit. Remove any unnecessary type tests, overly defensive checks, or testing of language features (e.g., testing that TypeScript generics work). Keep real business logic tests. Run the test suite after cleanup."

# Step 3: Verify
the agent -p "Run the full build, lint, type check, and test suite. Fix any failures. Do not add new features."

# Step 4: Commit
the agent -p "Create a conventional commit for all staged changes. Use 'feat: add OAuth2 login flow' as the message."
```

### Key Design Principles

1. **Each step is isolated**: A fresh context window per `the agent -p` call means no context bleed between steps.
...
the agent -p --model opus "Analyze the codebase architecture and write a plan for adding caching..."

# Implement with Sonnet (fast, capable)
the agent -p "Implement the caching layer according to the plan in docs/caching-plan.md..."

# Review with Opus (thorough)
the agent -p --model opus "Review all changes for security issues, race conditions, and edge cases..."
```

**With environment context:**
```bash
# Pass context via files, not prompt length
echo "Focus areas: auth module, API rate limiting" > .the agent-context.md
the agent -p "Read .the agent-context.md for priorities. Work through them in order."
rm .the agent-context.md
```

**With `--allowedTools` restrictions:**
```bash
# Read-only analysis pass
the agent -p --allowedTools "Read,Grep,Glob" "Audit this codebase for security vulnerabilities..."

# Write-only implementation pass
the agent -p --allowedTools "Read,Write,Edit,Bash" "Implement the fixes from security-audit.md..."
```: -

## 2. NanoClaw REPL

**ECC's built-in persistent loop.** A session-aware REPL that calls `the agent -p` synchronously with full conversation history.
...
1. Loads conversation history from `~/.opencode/claw/{session}.md`
2. Each user message is sent to `the agent -p` with full history as context
...
### Implementation via the coding agent Commands
...
## 4. Continuous The agent PR Loop
...
│                                                     │
│  1. Create branch (continuous-the agent/iteration-N)   │
│  2. Run the agent -p with enhanced prompt              │
│  3. (Optional) Reviewer pass — separate the agent -p   │
│  4. Commit changes (the agent generates message)       │
│  5. Push + create PR (gh pr create)                 │
│  6. Wait for CI checks (poll gh pr checks)          │
│  7. CI failure? → Auto-fix pass (the agent -p)         │
...
> **Warning:** Install continuous-the agent from its repository after reviewing the code. Do not pipe external scripts directly to bash.

### Usage

```bash
# Basic: 10 iterations
continuous-the agent --prompt "Add unit tests for all untested functions" --max-runs 10

# Cost-limited
continuous-the agent --prompt "Fix all linter errors" --max-cost 5.00

# Time-boxed
continuous-the agent --prompt "Improve test coverage" --max-duration 8h
...
continuous-the agent --prompt "Add tests" --max-runs 5 --worktree tests-worker &
continuous-the agent --prompt "Refactor code" --max-runs 5 --worktree refactor-worker &
wait
```
...
The agent reads this file at iteration start and updates it at iteration end. This bridges the context gap between independent `the agent -p` invocations.

### CI Failure Recovery

When PR checks fail, Continuous The agent automatically:
1. Fetches the failed run ID via `gh run list`
2. Spawns a new `the agent -p` with CI fix context
...
the agent -p "Implement the feature with full TDD. Be thorough with tests."

# Step 2: De-sloppify (separate context, focused cleanup)
the agent -p "Review all changes in the working tree. Remove:
...
for feature in "${features[@]}"; do
  # Implement
  the agent -p "Implement $feature with TDD."

  # De-sloppify
  the agent -p "Cleanup pass: review changes, remove test/code slop, run tests."

  # Verify
  the agent -p "Run build + lint + tests. Fix any failures."

  # Commit
  the agent -p "Commit with message: feat: add $feature"
done
```
...
| Multi-day project | Yes | Maybe (continuous-the agent) |
...
└─ No → Is there a written spec/RFC?
         ...
         │        └─ No → Continuous The agent (iterative PR loop)
...
2. **Continuous The agent + De-Sloppify**: Add `--review-prompt` with a de-sloppify directive to each iteration.
...
4. **Ralphinho's tiered approach in simpler loops**: Even in a sequential pipeline, you can route simple tasks to Haiku and complex tasks to Opus:
   ...
   the agent -p --model haiku "Fix the import ordering in src/utils.ts"

   # Complex architectural change
   the agent -p --model opus "Refactor the auth module to use the strategy pattern"
   ```: -
...
2. **No context bridge between iterations**: Each `the agent -p` call starts fresh. Use `SHARED_TASK_NOTES.md` or filesystem state to bridge context.
...
| Continuous The agent | AnandChowdhary | credit: @AnandChowdhary |
