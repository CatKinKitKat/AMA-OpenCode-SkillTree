...
> **Portability:** Requires `web_fetch` (Google Patents, Espacenet, USPTO), `WebSearch` (adjacent academic art), Node.js with `docx` package, and optionally Lens.org API key for citation-graph signals. Works in the coding agent CLI natively. In the agent platform with web tools + Code Execution + BYOK Lens.org, the workflow is supported.
