...
- Reps are citing "Maria approved 28% on the-project last quarter" as precedent and you need to break the precedent loop
...
- **Letting precedent set policy.** "Maria approved 28% on the-project last quarter" is not a band: it's an exception that didn't break the policy. `exception_router.py` flags 3+ similar exceptions as a signal that **the matrix is wrong**, not the deal. (Anti-pattern AP-1.)
