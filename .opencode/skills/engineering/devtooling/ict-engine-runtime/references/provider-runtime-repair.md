...
- the agent terminal() may run non-login bash and resolve `python3` differently from the operator zsh. If bash shows `python3_provider_dependencies_missing` but zsh has deps, rerun provider checks through `zsh -lc 'cd ~/projects-ict-engine/ict-engine && ...'` or use `~/.venvs/ict-engine-provider-py313/bin/python` for fetch scripts. Treat this as shell/runtime parity, not provider failure.
