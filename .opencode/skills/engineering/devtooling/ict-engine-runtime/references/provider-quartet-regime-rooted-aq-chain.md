...
If `uv run --with pandas --with numpy --with catboost ...` fails on PyPI/TLS/DNS but CatBoost is already installed in the user Python, use `~/.local/bin/python3` for both train and apply passes. This host has CatBoost under `~/Library/Python/3.13/lib/python/site-packages`. Do not stop at uv network failure.
