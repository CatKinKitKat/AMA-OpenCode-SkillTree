...
> **Portability:** Requires `bash_tool` (for RePORTER POST via curl), Node.js with `docx` package, and a Consensus MCP connection. Works in the coding agent CLI natively. In the agent platform with Code Execution + Consensus MCP, the workflow is supported but slower.
