...
Let `<the agent-home>` denote the the coding agent home directory: `~/.the agent` on macOS/Linux, `%USERPROFILE%\.the agent` on Windows. Resolve it the way the host platform resolves the user home directory (do not hardcode `~`).

| Form | Detection | `{ORCH_CMD}` | Agent name format |
|: -|: -|: -|: -|
| Plugin install (1.9.0+) | `<the agent-home>/plugins/marketplaces/everything-the agent-code/` exists | `/everything-the agent-code:orchestrate` | `everything-the agent-code:<name>` |
| Legacy bare install | Above absent. Agent files under `<the agent-home>/agents/` | `/orchestrate` | `<name>` |

Why this matters: under the plugin install, agents register as `everything-the agent-code:tdd-guide`. Bare names force fuzzy matching, which fails intermittently under parallel calls. Under legacy, the prefixed forms are not registered and fail outright.
...
2. Detect ECC install form once and freeze it into `ECC_MODE`. Algorithm (run in order, stop at the first match):
   1. If `<the agent-home>/plugins/marketplaces/everything-the agent-code/` exists → `ECC_MODE=plugin`.
   2. Else if `<the agent-home>/agents/` exists and contains at least one ECC agent file (e.g. `tdd-guide.md`, `code-reviewer.md`) → `ECC_MODE=legacy`.
...
5. **Normalize any agent names declared in the plan**: if the plan text references agents by their plugin-prefixed form (e.g. `everything-the agent-code:tdd-guide`), strip the prefix to get the bare catalogue name before validating or composing chains. Re-prefixing happens only at output time per `ECC_MODE` (Phase 4). Never let a pre-prefixed name flow into chain composition: it would double-prefix in plugin mode.
...
- `{ORCH_CMD}` = `/everything-the agent-code:orchestrate` under `plugin`, `/orchestrate` under `legacy`.
- `{AGENT(name)}` = `everything-the agent-code:<name>` under `plugin`, `<name>` under `legacy`.
...
- [ ] Every agent in every chain comes from the catalogue (after stripping any `everything-the agent-code:` prefix that appeared in the plan; see Phase 0 step 5).
...
- **Plan declares agents** (rare): first **strip any `everything-the agent-code:` prefix** to get the bare catalogue name (Phase 0 step 5), then validate against the catalogue. Replace invalid agents and explain under "Chain rationale". The bare name is re-prefixed at output time per `ECC_MODE`.
...
 5 bash
/everything-the agent-code:orchestrate custom "everything-the agent-code:tdd-guide,everything-the agent-code:database-reviewer,everything-the agent-code:python-reviewer,everything-the agent-code:security-reviewer" "[Plan: docs/plan/example-feature.md#step-2] Implement EncryptedString SQLAlchemy type and migrate UserProfile.birth_datetime/location columns; key from ENV APP_DB_KEY; Acceptance: encrypt/decrypt roundtrip tests pass; alembic upgrade/downgrade clean on empty DB; no plaintext in DB after migrate; Out of scope: cross-tenant profile sharing logic"
 6 `
