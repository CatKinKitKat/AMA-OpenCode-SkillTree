...
**Created by**: `ralph-loop.py` (the coding agent only)
...
```json
{
  ...
  ".opencode/hooks/session-start.py": "0a9899e80f6bfe15...",
  ".opencode/commands/start.md": "d1276dcbff880299..."
}
```
...
**Injected by**: `session-start.py` hook (the coding agent)
...
→ See `the agent-code/worktree-config.md` for details.
...
**Purpose**: Store the coding agent session ID for resume.
...
→ See `core/scripts.md` and `the agent-code/scripts.md`
...
| `.opencode/hooks/*.py` | Hook scripts |
