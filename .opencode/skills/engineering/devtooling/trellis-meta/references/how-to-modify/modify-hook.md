...
|------|--------|----------|
| `.opencode/hooks/{hook}.py` | Modify | Yes |
...
Edit `.opencode/hooks/session-start.py`:
...
Edit `.opencode/hooks/inject-subagent-context.py`:
...
Edit `.opencode/hooks/ralph-loop.py`:
...
echo '{"subagent_type":"check","output":"test"}' | \
  python3 .opencode/hooks/ralph-loop.py
```

### Integration Test

1. Start new the coding agent session
