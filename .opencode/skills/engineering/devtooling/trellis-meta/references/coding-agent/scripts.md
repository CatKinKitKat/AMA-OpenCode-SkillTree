# the coding agent Scripts

Scripts that require the coding agent CLI and hook system.
...
**Options:**
- `--name` - Task slug
- `--type` - `engineering/frontend`, `engineering/backend`, `fullstack`
- `--requirement` - Task description

**Actions:**
1. Creates task directory
2. Launches Plan Agent via `the agent`
...
7. Start The agent Dispatch Agent:
   ```bash
   the agent -p --agent dispatch \
     --session-id <uuid> \
...
```bash
the agent --agent dispatch "Start the pipeline"
```

### Print Mode (non-interactive)

```bash
the agent -p "Do something"
```

### Session Resume

```bash
the agent --resume <session-id>
```

### Automation Mode

```bash
the agent --dangerously-skip-permissions -p "..."
```

### JSON Output

```bash
the agent --output-format stream-json -p "..."
```
