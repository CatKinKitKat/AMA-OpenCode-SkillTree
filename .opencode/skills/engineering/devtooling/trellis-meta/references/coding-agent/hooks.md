...
the coding agent hooks for automatic context injection and quality enforcement.

---

## Overview

Hooks intercept the coding agent lifecycle events to inject context and enforce quality.
    ...
    "SessionStart": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "python3 \"$AGENT_PROJECT_DIR/.opencode/hooks/session-start.py\"",
            "timeout": 10
          }
        ]
      }
    ...
    "PreToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "python3 \"$AGENT_PROJECT_DIR/.opencode/hooks/inject-subagent-context.py\"",
            "timeout": 30
          }
        ]
      }
    ...
    "SubagentStop": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "python3 \"$AGENT_PROJECT_DIR/.opencode/hooks/ralph-loop.py\"",
            "timeout": 300
          }
        ]
      }
...
Inject initial context when a the coding agent session starts.
...
|----------|-------------|
| `AGENT_PROJECT_DIR` | Project root directory |
...
cat .opencode/settings.json | grep -A 20 '"hooks"'

# Test session-start manually
python3 .opencode/hooks/session-start.py

# Test inject-context (needs TOOL_INPUT env var)
TOOL_INPUT='{"subagent_type":"implement","prompt":"test"}' \
  python3 .opencode/hooks/inject-subagent-context.py
```
