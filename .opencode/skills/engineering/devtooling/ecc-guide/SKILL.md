...
Use this skill when a user needs help understanding, navigating, installing, or choosing parts of Everything the coding agent.
...
```bash
node scripts/install-plan.js --list-profiles
node scripts/install-plan.js --profile minimal --target the agent --json
node scripts/install-apply.js --profile minimal --target the agent --dry-run
```

For specific skill installs:

```bash
node scripts/install-plan.js --skills <skill-id> --target the agent --json
node scripts/install-apply.js --skills <skill-id> --target the agent --dry-run
```
...
- `.opencode/`, `.cursor/`, `.codex/`, `.gemini/`, `.opencode/`, `.codebuddy/`, `.joycode/`, or `.qwen/`
- `hooks/hooks.json`
- install-state files
...
- `/security-scan`: inspect The agent/OpenCode configuration security
