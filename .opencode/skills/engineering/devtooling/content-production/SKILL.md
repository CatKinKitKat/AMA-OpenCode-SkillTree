...
If `.opencode/product-marketing-context.md` exists, read it before asking questions. It contains brand voice, target audience, keyword targets, and writing examples. Use what's there: only ask for what's missing.
...
Check the draft against the brand's voice profile (from `.opencode/product-marketing-context.md`):
