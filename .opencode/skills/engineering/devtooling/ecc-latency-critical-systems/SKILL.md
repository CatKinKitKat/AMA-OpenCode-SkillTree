...
- p50, p95, and p99 latency. - throughput. - freshness age. - queue depth. - cache hit rate. - provider/API response time. - browser render time. - correctness under load. - failure and retry behavior.
...
- HTTP timing and response headers. - provider freshness timestamp. - queue or job state. - edge/cache state. - browser verification for actual UI freshness. - logs around retries and degraded mode.
