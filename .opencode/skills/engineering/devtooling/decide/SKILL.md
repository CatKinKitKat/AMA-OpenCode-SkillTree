...
1. **Raw transcripts**: every boardroom session, every advisor's Phase 2 position, every dissent. Stored under `~/.opencode/decisions/raw/`. Reference only, never feeds back automatically.
2. **Approved decisions**: only the founder-signed memos. Stored under `~/.opencode/decisions/approved/`. Feeds into future `/cs:office-hours` and `/cs:founder-mode` calls.
...
4. Append to `~/.opencode/decisions/approved/<YYYY-MM-DD>-<slug>.md`
