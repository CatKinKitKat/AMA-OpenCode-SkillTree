...
**Location:** `~/.opencode/decisions/raw/YYYY-MM-DD-<slug>.md`
...
- Archive after 90 days → `~/.opencode/decisions/raw/archive/YYYY/`

### Layer 2: Approved Decisions
**Location:** `~/.opencode/decisions/approved/`: one record per decision (`YYYY-MM-DD-<slug>.md`) plus the append-only index `decisions.md`
...
Migration: a legacy `memory/board-meetings/` folder may exist from earlier versions. Read it for history but write all new entries to `~/.opencode/decisions/`.: -
...
**Raw transcript:** ~/.opencode/decisions/raw/[DATE]-<slug>.md
```: -
...
2. Write Layer 1 raw transcript → `~/.opencode/decisions/raw/YYYY-MM-DD-<slug>.md`
3. Check conflicts against `~/.opencode/decisions/approved/decisions.md`
4. Surface conflicts → wait for founder resolution
5. Write the approved record to `~/.opencode/decisions/approved/YYYY-MM-DD-<slug>.md` and append to the index `decisions.md`
