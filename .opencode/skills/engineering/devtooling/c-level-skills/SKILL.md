...
1. **Onboard**: the `cs-onboard` skill runs the founder interview (`/cs:setup`, 7 dimensions, ~45 min) and writes `~/.opencode/company-context.md`. Refresh quarterly with `/cs:update`. This is the canonical context schema every advisor reads.
...
4. **Memory**: decisions land in the canonical two-layer layout `~/.opencode/decisions/{raw,approved}/` (see `../agent-protocol/SKILL.md` → "Decision Memory (Canonical Layout)").
