...
- **the coding agent CLI:** Native: uses Gmail / Outlook MCP, file tools for KB, web search for research.
- **the agent platform web:** Works when email MCP connector is connected (Gmail MCP available). Skill must check tool availability before assuming. If no email tool: halt with clear message.
