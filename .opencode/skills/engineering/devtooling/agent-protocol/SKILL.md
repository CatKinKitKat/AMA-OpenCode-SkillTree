...
**Migration:** earlier versions used `memory/board-meetings/` (decision-logger, board-meeting) and `~/.opencode/decision-log.md` (chief-of-staff). Read those for history if present, but write all new entries to `~/.opencode/decisions/`.
