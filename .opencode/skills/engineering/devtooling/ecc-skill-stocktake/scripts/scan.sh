...
#
# Environment:
#   SKILL_STOCKTAKE_GLOBAL_DIR   Override ~/.opencode/skills (for testing only;
#                                do not set in production — intended for bats tests)
#   SKILL_STOCKTAKE_PROJECT_DIR  Override project dir detection (for testing only)

set -euo pipefail

GLOBAL_DIR="${SKILL_STOCKTAKE_GLOBAL_DIR:-$HOME/.opencode/skills}"
CWD_SKILLS_DIR="${SKILL_STOCKTAKE_PROJECT_DIR:-${1:-$PWD/.opencode/skills}}"
...
OBSERVATIONS="${SKILL_STOCKTAKE_OBSERVATIONS:-$HOME/.opencode/observations.jsonl}"
