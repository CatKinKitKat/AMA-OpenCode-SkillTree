...
description: Interactive installer for Everything the coding agent — guides users through selecting and installing skills and rules to user-level or project-level directories, verifies paths, and optionally optimizes installed files.
origin: ECC
---


# Configure Everything the coding agent (ECC)

An interactive, step-by-step installation wizard for the Everything the coding agent project. Uses `AskUserQuestion` to guide users through selective installation of skills and rules, then verifies correctness and offers optimization.

## When to Activate

- User says "configure ecc", "install ecc", "setup everything the coding agent", or similar
...
2. **Manual**: Copy only this skill to `~/.opencode/skills/configure-ecc/SKILL.md`, then activate by saying "configure ecc": -
...
```bash
rm -rf /tmp/everything-the agent-code
git clone https://github.com/affaan-m/everything-the agent-code.git /tmp/everything-the agent-code
```

Set `ECC_ROOT=/tmp/everything-the agent-code` as the source for all subsequent copy operations.
...
Options:
  - "User-level (~/.opencode/)" — "Applies to all your the coding agent projects"
...
- User-level: `TARGET=~/.the agent`
- Project-level: `TARGET=.the agent` (relative to current project root)
- Both: `TARGET_USER=~/.the agent`, `TARGET_PROJECT=.the agent`
...
Options:
  ...
  - "Research & APIs" — "Deep research, Exa search, The agent API patterns"
...
`the assistant-api` is an Anthropic canonical skill. Install it from [`anthropics/skills`](https://github.com/anthropics/skills) when you want the official the assistant API workflow instead of an ECC-bundled copy.
...
```bash
grep -rn "~/.opencode/" $TARGET/skills/ $TARGET/rules/
grep -rn "../common/" $TARGET/rules/
grep -rn "skills/" $TARGET/skills/
```

**For project-level installs**, flag any references to `~/.opencode/` paths:
- If a skill references `~/.opencode/settings.json`: this is usually fine (settings are always user-level)
- If a skill references `~/.opencode/skills/` or `~/.opencode/rules/`: this may be broken if installed only at project level
...
- `continuous-learning-v2` references `~/.opencode/homunculus/` directory
...
3. **Issue**: What's wrong (e.g., "references ~/.opencode/skills/python-patterns but python-patterns was not installed")
...
- For user-level: check `~/.opencode/skills/<skill-name>/SKILL.md` exists
...
- Some skills assume `~/.opencode/` paths. Run Step 4 verification to find and fix these.
- For `continuous-learning-v2`, the `~/.opencode/homunculus/` directory is always user-level: this is expected and not an error.
