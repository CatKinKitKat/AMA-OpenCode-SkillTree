...
1. **`the agent agents` command** (primary): run `the agent agents` to get all agents known to the CLI, including user agents, plugin agents (e.g. `everything-the agent-code:architect`), and built-in agents. This automatically covers ECC marketplace installs without any path configuration.
2. **File glob** (fallback, for reading agent content): agent markdown files are read from:
   ...
   - `~/.opencode/agents/**/*.md` + `~/.opencode/agents/*.md`: global user agents
...
Run `the agent agents` to get the full agent list. Parse each line:
- **Plugin agents** are prefixed with `plugin-name:` (e.g., `everything-the agent-code:security-reviewer`). Use the part after `:` as the agent name and the plugin name as the domain.
- **User agents** have no prefix. Read the corresponding markdown file from `~/.opencode/agents/` or `./agents/` to extract the name and description.
...
If no agents are found after running `the agent agents` and probing file locations, inform the user: "No agents found. Run `the agent agents` to verify your setup." Then stop.
...
- **Parallel Agent calls, not TeamCreate.** This skill uses parallel Agent tool calls for independent work. TeamCreate (a the coding agent tool for multi-agent dialogue) is only needed when agents must debate or respond to each other.
