#!/usr/bin/env bash
# One-shot migration from the legacy The agent config tree into the
# continuous-learning-v2 data directory.
set -euo pipefail

OLD="${HOME}/.opencode/homunculus"
...
if command -v pgrep >/dev/null 2>&1; then
  if pgrep -f "${HOME}.*observer-loop\\.sh" >/dev/null 2>&1; then
    ...
    echo "Exit all the coding agent sessions, then re-run." >&2
    exit 1
  fi
else
...
settings="${HOME}/.opencode/settings.json"
if [ -f "$settings" ] && grep -q '"CLV2_CONFIG"' "$settings" 2>/dev/null; then
  if grep -q '\.opencode/homunculus' "$settings" 2>/dev/null; then
    cat >&2 <<WARN
