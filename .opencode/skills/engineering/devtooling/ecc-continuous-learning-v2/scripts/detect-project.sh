...
_clv2_detect_project() {
  ...
  # 1. Try AGENT_PROJECT_DIR env var
  if [ -n "$AGENT_PROJECT_DIR" ] && [ -d "$AGENT_PROJECT_DIR" ] && command -v git &>/dev/null; then
    project_root=$(git -C "$AGENT_PROJECT_DIR" rev-parse --show-toplevel 2>/dev/null || true)
  ...
  # Normalize Windows backslashes so basename works when AGENT_PROJECT_DIR
