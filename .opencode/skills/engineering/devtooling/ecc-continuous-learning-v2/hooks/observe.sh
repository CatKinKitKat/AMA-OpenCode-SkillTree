...
# the coding agent passes hook data via stdin as JSON.
#
...
# Can also be registered manually in ~/.opencode/settings.json.

set -e

# Hook phase from CLI argument: "pre" (PreToolUse) or "post" (PostToolUse).
# Manual settings.json installs can call this script without the plugin
# wrapper's positional phase argument, but the coding agent still exposes the hook
...
# Read JSON from stdin (the coding agent hook format)
INPUT_JSON=$(cat)
...
if [ -n "$STDIN_CWD" ] && [ -d "$STDIN_CWD" ]; then
  ...
  if [ -n "$_GIT_ROOT" ]; then
    export AGENT_PROJECT_DIR="$_GIT_ROOT"
    unset CLV2_NO_PROJECT
  else
    unset AGENT_PROJECT_DIR
...
_ECC_SKIP_PATHS="${ECC_OBSERVE_SKIP_PATHS:-observer-sessions,.the agent-mem}"
if [ -n "$STDIN_CWD" ]; then
...
# Pass HOOK_PHASE via env var since the coding agent does not include hook type in stdin JSON
...
try:
    ...
    # the coding agent does NOT include a "hook_type" field in the stdin JSON,
    # so we rely on the shell argument ("pre" or "post") instead.
    hook_phase = os.environ.get("HOOK_PHASE", "post")
    event = "tool_start" if hook_phase == "pre" else "tool_complete"

    # Extract fields - the coding agent hook format
...
# which caused runaway parallel The agent analysis processes.
