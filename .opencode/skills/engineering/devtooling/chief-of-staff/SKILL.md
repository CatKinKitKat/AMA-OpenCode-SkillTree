...
- **Layer 1 (raw):** `~/.opencode/decisions/raw/YYYY-MM-DD-{slug}.md`: full deliberation transcript
- **Layer 2 (approved):** `~/.opencode/decisions/approved/YYYY-MM-DD-{slug}.md`: founder-approved decisions only
...
At session start: scan `~/.opencode/decisions/approved/`: if a review date has passed, flag it: *"You decided [X] on [date]. Worth a check-in?"*

Migration: a legacy single-file log at `~/.opencode/decision-log.md` may exist from earlier versions. Read it for history but write new entries to `~/.opencode/decisions/`.: -
