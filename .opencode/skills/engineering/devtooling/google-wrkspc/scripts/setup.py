#!/usr/bin/env python3
"""Google Workspace OAuth2 setup for the agent.
...
# Ensure sibling modules (_agent_home) are importable when run standalone.
_SCRIPTS_DIR = str(Path(__file__).resolve().parent)
if _SCRIPTS_DIR not in sys.path:
    sys.path.insert(0, _SCRIPTS_DIR)

from _agent_home import display_agent_home, get_agent_home

AGENT_HOME = get_agent_home()
TOKEN_PATH = AGENT_HOME / "google_token.json"
CLIENT_SECRET_PATH = AGENT_HOME / "google_client_secret.json"
PENDING_AUTH_PATH = AGENT_HOME / "google_oauth_pending.json"
...
def _format_missing_scopes(missing_scopes: list[str]) -> str:
    ...
    return (
        ...
        f"{bullets}\n"
        "Run the Google Workspace setup again from this same the agent profile to refresh consent."
    )
...
def install_deps():
    ...
    try:
        ...
        print("  pip install 'agent-runtime[google]'")
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace OAuth setup for the agent")
