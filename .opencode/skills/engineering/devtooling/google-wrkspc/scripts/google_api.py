#!/usr/bin/env python3
"""Google Workspace API CLI for the agent.

Uses the Google Workspace CLI (`gws`) when available, but preserves the
existing the agent-facing JSON contract and falls back to the Python client
...
from _agent_home import get_agent_home
...
def _gws_binary() -> str | None:
    override = os.getenv("AGENT_GWS_BIN")
    if override:
        return override
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace API for the agent")
