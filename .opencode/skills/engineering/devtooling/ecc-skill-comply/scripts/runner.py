"""Run scenarios via the agent -p and parse tool calls from stream-json output."""
...
def run_scenario(
    ...
    # the agent -p returns rc=1 when --max-turns is reached, but the stream-json
    ...
    if result.returncode != 0 and not nonfatal_max_turns:
        # Include both stderr and stdout tails. the agent -p often surfaces the
        ...
        raise RuntimeError(
            f"the agent -p failed (rc={result.returncode}): "
...
def _parse_stream_json(stdout: str) -> list[ObservationEvent]:
    """Parse the agent -p stream-json output into ObservationEvents.
