...
def generate_scenarios(
    ...
    Calls the agent -p with the scenario_generator prompt, parses YAML output.
    """
    ...
    result = subprocess.run(
        ["the agent", "-p", prompt, "--model", model, "--output-format", "text"],
        capture_output=True,
        text=True,
        timeout=120,
    )
    ...
    if result.returncode != 0:
        raise RuntimeError(f"the agent -p failed: {result.stderr}")

    if not result.stdout.strip():
        raise RuntimeError("the agent -p returned empty output")
