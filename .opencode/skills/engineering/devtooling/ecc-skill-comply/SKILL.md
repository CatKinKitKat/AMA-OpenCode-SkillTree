...
3. Running `the agent -p` and capturing tool call traces via stream-json
...
```bash
# Full run
uv run python -m scripts.run ~/.opencode/rules/common/testing.md

# Dry run (no cost, spec + scenarios only)
uv run python -m scripts.run --dry-run ~/.opencode/skills/search-first/SKILL.md
