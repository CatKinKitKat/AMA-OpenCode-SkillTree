...
3. Detect skill type:
   ...
   - Has `.the agent-plugin/plugin.json` → is a standalone plugin
...
Auditing: code-to-prd
  ...
  Plugin: .the agent-plugin/plugin.json (found)
```: -
...
If `{skill_path}/.the agent-plugin/plugin.json` exists:
...
Check if the skill has an entry in `.the agent-plugin/marketplace.json`:
...
Check the parent domain's `.the agent-plugin/plugin.json`:
