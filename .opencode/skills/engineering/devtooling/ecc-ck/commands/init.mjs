 ...
 * init.mjs — auto-detect project info and output JSON for The agent to confirm
 *
 * Usage: node init.mjs
...
// ── AGENTS.md ─────────────────────────────────────────────────────────────────
const the agentMd = readFile('AGENTS.md');
if (the agentMd) {
  const goal = extractSection(the agentMd, 'Current Goal');
  if (goal && !output.goal) output.goal = goal.split('\n')[0].trim();

  const doNot = extractSection(the agentMd, 'Do Not Do');
  if (doNot) {
  ...
  const stack = extractSection(the agentMd, 'Tech Stack');
  if (stack && output.stack.length === 0) {
    output.stack = stack.split(/[,\n]/).map(s => s.replace(/^[-*]\s+/, '').trim()).filter(Boolean);
  }

  // Description from first section or "What This Is"
  const whatItIs = extractSection(the agentMd, 'What This Is') || extractSection(the agentMd, 'About');
