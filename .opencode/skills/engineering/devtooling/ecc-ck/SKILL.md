---
name: ecc-ck
description: Persistent per-project memory for the coding agent. Auto-loads project context on session start, tracks sessions with git activity, and writes to native memory. Commands run deterministic Node.js scripts — behavior is consistent across model versions.
origin: community
version: 2.0.0
author: sreedhargs89
...
Scripts live at: `~/.opencode/skills/ck/commands/` (expand `~` with `$HOME`).: -
...
```bash
node "$HOME/.opencode/skills/ck/commands/init.mjs"
```
...
```bash
echo '<confirmed-json>' | node "$HOME/.opencode/skills/ck/commands/save.mjs" --init
```
...
```bash
echo '<json>' | node "$HOME/.opencode/skills/ck/commands/save.mjs"
```
...
```bash
node "$HOME/.opencode/skills/ck/commands/resume.mjs" [arg]
```
...
```bash
node "$HOME/.opencode/skills/ck/commands/info.mjs" [arg]
```
Display output verbatim. No follow-up question.: -

### `/ck:list`: Portfolio View
```bash
node "$HOME/.opencode/skills/ck/commands/list.mjs"
```
...
If yes:
```bash
node "$HOME/.opencode/skills/ck/commands/forget.mjs" [name]
```
Display confirmation verbatim.: -

### `/ck:migrate`: Convert v1 Data to v2
```bash
node "$HOME/.opencode/skills/ck/commands/migrate.mjs"
```
For a dry run first:
```bash
node "$HOME/.opencode/skills/ck/commands/migrate.mjs" --dry-run
```
...
The hook at `~/.opencode/skills/ck/hooks/session-start.mjs` must be registered in
`~/.opencode/settings.json` to auto-load project context on session start:

```json
{
  "hooks": {
    "SessionStart": [
      { "hooks": [{ "type": "command", "command": "node \"~/.opencode/skills/ck/hooks/session-start.mjs\"" }] }
    ]
  }
}
```
