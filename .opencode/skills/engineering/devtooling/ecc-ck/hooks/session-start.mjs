...
function extractThe agentMdGoal(projectPath) {
...
function main() {
  ...
  if (entry?.contextDir) {
    ...
    if (context) {
      ...
      const the agentMdGoal = extractThe agentMdGoal(cwd);
      if (the agentMdGoal && context.goal &&
          the agentMdGoal.toLowerCase().trim() !== context.goal.toLowerCase().trim()) {
        summaryLines.push(`WARNING Goal mismatch — ck: "${context.goal.slice(0, 40)}" · AGENTS.md: "${the agentMdGoal.slice(0, 40)}"`);
      ...
      // Instruct The agent to display compact briefing at session start
      parts.push([
        `---`,
        `## ck: SESSION START`,
        ``,
