---
name: git-guardrails-the agent-code
description: Set up the coding agent hooks to block dangerous git commands (push, reset --hard, clean, branch -D, etc.) before they execute. Use when user wants to prevent destructive git operations, add git safety hooks, or block git push/reset in the coding agent.
---
    ...
    "PreToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "\"$AGENT_PROJECT_DIR\"/.opencode/hooks/block-dangerous-git.sh"
          }
        ]
      }
    ]
