...
- Quick single-file fixes (use standard `the agent -p`)
...
### Via the coding agent (Manual)

```bash
# Step 1: Plan
the agent -p --model opus "You are a Product Planner. Read PLANNER_PROMPT.md. Expand this brief into a full product spec: 'Build a Kanban board app'. Write spec to spec.md"

# Step 2: Generate (iteration 1)
the agent -p --model opus "You are a Generator. Read spec.md. Implement Sprint 1. Start the dev server on port 3000."

# Step 3: Evaluate (iteration 1)
the agent -p --model opus --allowedTools "Read,Bash,mcp__playwright__*" "You are an Evaluator. Read EVALUATOR_PROMPT.md. Test the live app at http://localhost:3000. Score against the rubric. Write feedback to feedback-001.md"

# Step 4: Generate (iteration 2 — reads feedback)
the agent -p --model opus "You are a Generator. Read spec.md and feedback-001.md. Address all issues. Improve the scores."
...
6. **Context exhaustion**: For long sessions, use The agent Agent SDK's automatic compaction or reset context between major phases.
