---
name: ecc-agent-imports
description: Convert local the agent operator workflows into sanitized ECC skills and release-pack artifacts. Use when preparing a the agent workflow for public ECC reuse without leaking private workspace state, credentials, or local-only paths.
origin: ECC
---
...
Use this skill when turning a repeated the agent workflow into something safe to ship in ECC.

the agent is the operator shell. ECC is the reusable workflow layer. Imports should move stable patterns from the agent into ECC without moving private state.

## When To Use

- A the agent workflow has repeated enough times to become reusable.
...
- `~/.agent` paths unless the doc is explicitly explaining local setup
