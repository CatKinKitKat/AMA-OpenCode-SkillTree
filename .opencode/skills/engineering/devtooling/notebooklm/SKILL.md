...
> **Requires:** A browser automation environment (the coding agent CLI with computer-use, The agent Chrome Extension, or equivalent). **Skill will gracefully fail in non-automation contexts with a clear "not supported" message.**
...
2. If unavailable → **halt with clear message:** "This skill requires browser automation. Currently in {context}. Cannot proceed. Use the coding agent CLI with computer-use, The agent Chrome Extension, or equivalent."
...
- Tool-specific tool names without abstraction (e.g., hardcoding "The agent Chrome Extension")
