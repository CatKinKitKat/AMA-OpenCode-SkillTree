...
├── .opencode/commands/  # Slash commands: user-facing CLI
...
| Commands (`.opencode/commands/`) | User-facing slash commands (`/daily-sync`, `/outreach`) | Git-tracked |
...
`AGENTS.md` is the kernel. It acts as the COO / orchestrator. The agent reads it at session start and uses it to route work.
...
- Test runner access
- MCP servers as configured in `.opencode/mcp.json`
...
For parallel execution, use the coding agent's background task capability or shell scripts that invoke the coding agent with specific agent contexts.

## Commands and Daily Workflows

Slash commands are markdown files in `.opencode/commands/`. They define reusable workflows.
...
Place command files in `.opencode/commands/<command-name>.md`. the coding agent auto-discovers them. Users invoke them with `/<command-name>`.
...
Agentic OS tasks run on a schedule using external cron, not the coding agent's built-in cron (which dies when the session ends).
...
[Service]
Type=oneshot
ExecStart=/usr/local/bin/the agent --cwd /path/to/project --command /daily-sync
```
...
- [ ] Scheduled tasks use external cron (LaunchAgent, systemd, pm2), not the coding agent's session cron
