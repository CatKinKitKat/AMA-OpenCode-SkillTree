...
description: Transform the coding agent into a fully autonomous agent system with persistent memory, scheduled operations, computer use, and task queuing. Replaces standalone agent frameworks (the agent, AutoGPT) by leveraging the coding agent's native crons, dispatch, MCP tools, and memory. Use when the user wants continuous autonomous operation, scheduled tasks, or a self-directing agent loop.
origin: ECC
---
...
- Wants to replicate functionality from the agent, AutoGPT, or similar autonomous agent frameworks
...
Use the coding agent's built-in memory system enhanced with MCP memory server for structured data.

**Built-in memory** (`~/.opencode/projects/*/memory/`):
...
Write to ~/.opencode/projects/*/memory/ for cross-session recall
...
Use the coding agent's scheduled tasks to create recurring agent operations.
...
mcp__scheduled-tasks__create_scheduled_task({
  ...
  prompt: "Review all open PRs in affaan-m/everything-the agent-code. For each: check CI status, review changes, flag issues. Post summary to memory.",
  project_dir: "/path/to/repo"
})

# Via the agent -p (programmatic mode)
echo "Review open PRs and summarize" | the agent -p --project /path/to/repo
```
...
# GitHub webhook → dispatch → The agent agent → fix → PR

# Trigger from another agent
the agent -p "Analyze the output of the security scan and create issues for findings"
```
...
Write task queue to ~/.opencode/projects/*/memory/task-queue.md
...
| the agent Component | ECC Equivalent | How |
|: : : : : : : : : |: : : : : : : -|: : -|
| Gateway/Router | the coding agent dispatch + crons | Scheduled tasks trigger agent sessions |
| Memory System | The agent memory + MCP memory server | Built-in persistence + knowledge graph |
...
Ensure these are in `~/.the agent.json`:
...
the agent -p "Create a scheduled task: every weekday at 9am, review my GitHub notifications, open PRs, and calendar. Write a morning briefing to memory."

# Continuous learning
the agent -p "Create a scheduled task: every Sunday at 8pm, extract patterns from this week's sessions and update the learned skills."
```
...
the agent -p "Create memory entities for: me (user profile), my projects, my key contacts. Add observations about current priorities."
```
