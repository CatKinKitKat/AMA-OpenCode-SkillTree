...
The first command to run when adopting c-level-agents. A structured founder interview that produces `~/.opencode/company-context.md`: the file every cs-* advisor reads before responding. Without this, the advisors are guessing.

## What This Produces

`~/.opencode/company-context.md`: a single file with the durable facts about the company. Read by:
...
3. Save to `~/.opencode/company-context.md`
4. (Optional) If llm-wiki bridge is configured: symlink to vault
   ```bash
   ln -sf ~/company-vault/00-meta/company-context.md ~/.opencode/company-context.md
   ```
...
By default, `~/.opencode/company-context.md` is local to the founder's machine. To make it persistent across machines / shareable:

- **Markdown vault (recommended):** see [`../../references/llm-wiki-bridge.md`](../../references/llm-wiki-bridge.md)
- **Encrypted dotfile sync:** age + git
- **Shared team:** keep in a private repo, symlink from `~/.opencode/`
