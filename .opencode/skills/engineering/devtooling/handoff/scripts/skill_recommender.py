...
def _find_repo_root(start: Path) -> Path:
    """Walk parents and return the OUTERMOST dir containing AGENTS.md.

    Plugin folders also carry .the agent-plugin/, so we cannot stop at the
    ...
    if outermost is not None:
        return outermost
    for parent in [cur, *cur.parents]:
        if (parent / ".the agent-plugin").exists():
            return parent
    return start
