#!/usr/bin/env python3
"""UZI-Skill 一键运行入口 — 适用于 the coding agent / Codex / Cursor / 命令行 / 任何 agent。
...
# ─── 路径设置（v3.3.1 · 兼容 the agent layout）───
# run.py 可能出现在两个位置：
#   1) repo 根目录（the coding agent / Codex / Cursor / dev）：SCRIPTS_DIR = ROOT/skills/deep-analysis/scripts
#   2) skill 目录里（the agent `agent skills install` 后只拉子目录）：SCRIPTS_DIR = ROOT/scripts
# 探测两种 layout · 不破坏现有行为
ROOT_DIR = Path(__file__).parent.resolve()
_layout_candidates = [
    ROOT_DIR / "skills" / "deep-analysis" / "scripts",  # repo root layout
    ROOT_DIR / "scripts",                               # the agent skill-dir layout
]
...
def _get_version() -> str:
    """v2.6 · Read version from .the agent-plugin/plugin.json so banner stays in sync."""
    try:
        import json
        manifest = ROOT_DIR / ".the agent-plugin" / "plugin.json"
        if manifest.exists():
