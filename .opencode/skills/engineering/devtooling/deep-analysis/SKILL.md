...
6. 根据用户选择：
   - `y` → 告诉用户按 README 安装章节的命令执行（the coding agent: `/plugin update stock-deep-analyzer` · git clone: `cd UZI-Skill && git pull` · the agent: `agent skills update wbh604/UZI-Skill/skills/deep-analysis`），然后继续用户原请求
...
<HARD-GATE>
从 v2.15.0 起，`skills/deep-analysis/personas/*.yaml` 有全 52 位投资者的 persona 定义: **12 个 flagship** 手写（巴菲特 / 芒格 / 格雷厄姆 / 费雪 / 林奇 / 木头姐 / 索罗斯 / 达里奥 /
...
非 The agent 模型（Codex/国产模型）尤其容易踩这个坑。stage2 不会自动检测事实正确性，
质量靠 agent 自查。
</HARD-GATE>
...
> **核心原则**：每个投资者的判断不是"跑公式"，而是 The agent 真正站在这个人的角度思考。规则引擎给出量化参考分，最终判断由你做。
>
...
非 The agent 平台跑 UZI-Skill 时常见问题已被代码层修复，但 agent 也要主动适配：
...
| 非 The agent agent_analysis 错乱 | stage2 调用 `lib.agent_analysis_validator.validate()` 写 `_agent_analysis_errors.json` | 跑完看 console 是否有 🔴 schema error，按提示修 |
