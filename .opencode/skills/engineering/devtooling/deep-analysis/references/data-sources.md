...
| `one_liner` 一句话定位 | web search "{name} 主营业务" | The agent 从 `stock_zygc_em` 生成 | The agent 从行业+市值 生成 |
...
The agent web search only，不走 akshare。prompt 模板在 `fetch_macro.py`。
...
| `upstream` 上游描述 | The agent 从 `stock_zygc_em` + web search 生成 |: |
...
| `lifecycle` 生命周期 | The agent 判断（导入/成长/成熟/衰退）|
...
| `intangible` / `switching` / `network` / `scale` | The agent 从业务描述 + 同行对比 评估 (1-10) |
...
| `catalyst` 催化剂 | The agent 从 timeline 提炼 |
...
| `positive_pct` 正面占比 | web search + The agent 情感分析 |
...
8 信号扫描全部靠 The agent web search，详见 `skills/trap-detector/references/eight-signals.md`。
...
| `reason` 理由 | The agent 从业务描述 生成 1 句 |
...
The agent **从 synthesis 自动生成 5 条**，模板：
