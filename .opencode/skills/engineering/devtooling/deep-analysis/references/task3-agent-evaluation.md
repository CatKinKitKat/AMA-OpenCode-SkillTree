...
> **核心原则**：规则引擎是参考材料，不是最终判断。每个投资者的观点必须经过 The agent 的"角色扮演式思考"。
...
5. **主 The agent 合并**：把 sub-agent 的结果覆盖到 `panel.json` 的对应投资者
...
3. **如果 sub-agent 和规则引擎分数差 > 30 分**，主 The agent 要在 synthesis 里标记为"分歧点"
