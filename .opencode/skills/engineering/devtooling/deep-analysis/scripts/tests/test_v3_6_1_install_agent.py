"""Regression for v3.6.1 · the agent 安装脚本（绕过 Skills Guard）.

背景：the agent Skills Guard 扫描器假阳性 168 条 · DANGEROUS · --force 也绕不过.
解法：提供 install-agent.sh · clone + symlink · 跳过 Hub 扫描.

测试覆盖：
1. install-agent.sh 存在 + 可执行
2. 脚本 bash 语法合法
...
6. README + INSTALL-AGENT.md 已指向新脚本
"""
...
def test_install_script_exists():
    assert SCRIPT.exists(), "install-agent.sh 必须在 repo root"
    # 可执行权限
    import stat
    mode = SCRIPT.stat().st_mode
    assert mode & stat.S_IXUSR, "install-agent.sh 必须可执行 (chmod +x)"
...
def test_install_script_cleans_old_hub_install():
    """user 之前 agent skills install 装过 · 必须先清理 · 否则 symlink 会失败."""
    body = SCRIPT.read_text(encoding="utf-8")
    assert "rm -rf" in body
    # 清理目标必须是 agent skills dir 下的 4 个 skill · 不能 rm 错地方
    assert "AGENT_SKILLS_DIR" in body


# ─── #4 · venv pip fallback ──────────────────────────────

def test_install_script_pip_fallback_when_venv_missing():
    """the agent venv 可能在 ~/.agent/venv 或 ~/.agent/.venv · 两种都试 · 都没用系统 pip."""
    body = SCRIPT.read_text(encoding="utf-8")
...
def test_install_agent_md_promotes_script():
    """INSTALL-AGENT.md 顶部必须显眼指向新脚本 · 不能继续推 agent skills install."""
    doc = (REPO_ROOT / "INSTALL-AGENT.md").read_text(encoding="utf-8")
    assert "install-agent.sh" in doc
    # 必须说明 Skills Guard 问题
    assert "Skills Guard" in doc
    assert "DANGEROUS" in doc
    # 指向 the agent 上游 issue
    assert "1006" in doc  # NousResearch/agent-agent#1006


def test_readme_links_to_install_script():
    """README 安装表必须更新 · 不能继续说 agent skills install 就行."""
    readme = (REPO_ROOT / "README.md").read_text(encoding="utf-8")
    assert "install-agent.sh" in readme
