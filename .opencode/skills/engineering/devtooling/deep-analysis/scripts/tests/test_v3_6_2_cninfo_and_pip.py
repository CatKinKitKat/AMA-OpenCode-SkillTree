...
#69 (@FrankHuy) · install-agent.sh 在 Linux 找不到 pip · akshare 装不上
...
4. install-agent.sh · 含 Python 版本预检 (>=3.10)
5. install-agent.sh · pip 探测级联 (venv → pip → pip3 → python -m pip)
6. install-agent.sh · 完全找不到 pip 时给清晰错误指引 · 不静默 fail
"""
...
# ─── #3 · install-agent.sh Python + pip 探测 ────────────
