...
def stage1(ticker: str) -> dict:
    ...
    返回 {ticker, raw, dims, panel, features} 供 The agent agent 审查。
    The agent 应该在 stage1 之后介入，用 sub-agent 逐组分析 51 评委，
    ...
    print(f"")
    print(f"   ⏸️  此时 The agent agent 应介入：")
...
def stage2(ticker: str) -> str:
    ...
    在 The agent agent 审查/覆盖 panel.json + 写入 agent_analysis.json 之后调用。
    读取 .cache 中的最新数据生成报告。
    ...
    # v2.6 · 校验 agent_analysis schema（特别针对非 The agent 模型的输出）
    if agent_analysis:
        try:
    ...
    if agent_analysis and agent_analysis.get("agent_reviewed"):
        ...
        print(f"   提示: The agent agent 应在 stage1 之后写入 .cache/{ti.full}/agent_analysis.json")
