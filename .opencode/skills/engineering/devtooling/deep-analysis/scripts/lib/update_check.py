...
设计：
- 读本地 `.the agent-plugin/plugin.json::version`
...
def _read_local_version() -> str | None:
    manifest = _PLUGIN_ROOT / ".the agent-plugin" / "plugin.json"
...
def handle_answer(answer: str, latest: str) -> str:
    ...
    if a in ("y", "yes", "是"):
        return (
            ...
            f"  the coding agent: /plugin update stock-deep-analyzer\n"
            f"  git clone: cd UZI-Skill && git pull\n"
            f"  the agent: agent skills update wbh604/UZI-Skill/skills/deep-analysis"
        )
    return "→ 好的，下次启动再问"
