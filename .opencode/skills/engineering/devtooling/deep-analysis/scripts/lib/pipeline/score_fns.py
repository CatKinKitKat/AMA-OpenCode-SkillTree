...
def generate_synthesis(raw: dict, dims_scored: dict, panel: dict, agent_analysis: dict | None = None) -> dict:
    ...
    # 非 The agent LLM 可能写出 signal=bullish 但 score=5 这种自相矛盾输出。
    ...
    return {
        ...
        "the agent_narrative_stub": {
            "_note": "以下字段已由 agent 覆盖" if ag.get("agent_reviewed") else "以下字段是脚本生成的占位，Task 4 中 The agent 必须根据原始数据重写",
