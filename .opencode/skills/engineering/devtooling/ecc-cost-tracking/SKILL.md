...
description: Track and report the coding agent token usage, spending, and budgets from a local cost-tracking database. Use when the user asks about costs, spending, usage, tokens, budgets, or cost breakdowns by project, tool, session, or date.
origin: community
---
...
Use this skill to analyze the coding agent cost and usage history from a local SQLite
database. It is intended for users who already have a cost-tracking hook or
plugin writing usage rows to `~/.the agent-cost-tracker/usage.db`.
...
test -f ~/.the agent-cost-tracker/usage.db && echo "Database found" || echo "Database not found"
```
...
| `session_id` | the coding agent session identifier |
...
```bash
sqlite3 ~/.the agent-cost-tracker/usage.db "
  SELECT
...
```bash
sqlite3 -header -column ~/.the agent-cost-tracker/usage.db "
