...
description: Use when a the agent Kanban worker wants to run Codex CLI as an isolated implementation lane while the agent keeps ownership of task lifecycle, reconciliation, testing, and handoff.
version: 1.0.0
author: the agent
license: MIT
metadata:
  agent:
    ...
    related_skills: [kanban-worker, codex, agent-runtime]
---
...
This skill defines the lightweight the agent+Codex dual-lane convention for Kanban workers. the agent is always the task owner: it calls `kanban_show`, decides whether Codex is appropriate, creates or selects an isolated workspace, starts and monitors Codex, reconciles any diff, runs verification, and writes the final `kanban_complete` or `kanban_block` handoff. Codex is an input lane only. Codex output is not a task completion signal, not a trusted reviewer, and not allowed to write durable Kanban state directly.

The convention exists so a the agent worker can use Codex for bounded implementation help without changing the dispatcher. The dispatcher must still spawn the agent workers. A worker may optionally spawn Codex inside its own run, then accept, partially accept, or reject the lane after independent review and tests.
...
- A bounded diff can be evaluated by the agent in one run.
- The repo can be copied or checked out in an isolated git worktree/branch.
- the agent can run the relevant tests itself after Codex exits.
...
1. the agent owns the Kanban lifecycle. Codex must never call `kanban_complete`, `kanban_block`, `kanban_create`, gateway messaging, or any the agent board CLI as a substitute for the worker.
2. the agent owns final acceptance. Treat Codex commits/diffs as untrusted patches until reviewed and verified.
3. the agent owns test execution. Codex may run tests, but those runs are advisory. Repeat required verification from the agent with the repo's canonical wrapper.
4. the agent owns safety. If Codex changes safety boundaries, risk gates, live trading behavior, or secrets handling, reject the lane even if tests pass.
5. the agent owns cleanup. Kill stuck Codex processes and remove temporary worktrees when they are no longer needed.
...
```bash
TASK_ID="${AGENT_KANBAN_TASK:-t_manual}"
REPO="/path/to/repo"
...
If the current Kanban workspace is already an isolated git worktree created for this task, you may create a sibling Codex branch inside it only if `git status --short` is clean except for intentional the agent edits. Otherwise create a separate temporary worktree and cherry-pick or copy accepted commits back after reconciliation.
...
Run these before spawning Codex. Missing Codex is a normal reason to skip the lane, not a task blocker if the agent can do the task directly.
...
the agent owns the Kanban lifecycle; do not call the agent kanban tools or messaging.
...
Do not use `--yolo` for prediction-market-bot or safety-sensitive repos. Prefer `--full-auto` inside the isolated worktree, then rely on the agent reconciliation.
...
- Explicit statement: the agent owns Kanban lifecycle. Codex is an input lane only.
...
- Verification commands Codex may run and commands the agent will run afterward.
...
the agent must perform this checklist before accepting any Codex lane result:

- [ ] `git -C <WORKTREE> status --short --branch` shows only expected files.
- [ ] `git -C <WORKTREE> diff --stat` and `git diff` were reviewed by the agent.
...
- [ ] the agent ran the canonical tests itself, using `scripts/run_tests.sh` for the agent or the repo's documented wrapper for other repos.
- [ ] Any Codex-run tests are listed separately from the agent-run tests.
- [ ] Accepted commits/diffs were applied to the the agent-owned workspace/branch.
...
```json
{
  "codex_lane": {
    ...
    "tests_run": [
      {"command": "scripts/run_tests.sh tests/tools/test_x.py", "exit_code": 0, "owner": "agent"},
...
```json
{
  "codex_lane": {
    ...
    "result": "rejected",
    "accepted_commits": [],
    "rejected_reason": "Direct the agent edit was smaller and safer than spawning Codex.",
    "tests_run": [],
    "artifacts": []
  }
}
...
1. Treating Codex self-report as verification. Always inspect the diff and rerun tests from the agent.
2. Running Codex in the user's dirty main checkout. Always isolate in a worktree/branch.
3. Letting Codex own Kanban. Codex may summarize progress, but the agent writes board state.
...
- [ ] the agent reviewed `git diff` and safety-sensitive files.
- [ ] the agent ran canonical tests independently.
