...
description: Write-time code quality enforcement using Plankton — auto-formatting, linting, and The agent-powered fixes on every file edit via hooks.
origin: community
---
...
Integration reference for Plankton (credit: @alxfazio), a write-time code quality enforcement system for the coding agent. Plankton runs formatters and linters on every file edit via PostToolUse hooks, then spawns The agent subprocesses to fix violations the agent didn't catch.
...
├─ Spawns the agent -p subprocess with violations JSON
...
No install command, no plugin config. The hooks in `.opencode/settings.json` are picked up automatically when you run the coding agent in the Plankton directory.
...
2. Copy `.opencode/settings.json` hook configuration
