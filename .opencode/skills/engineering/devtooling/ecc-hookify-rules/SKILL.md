...
Hookify rules are markdown files with YAML frontmatter that define patterns to watch for and messages to show when those patterns match. Rules are stored in `.opencode/hookify.{rule-name}.local.md` files.
...
- **Location**: `.opencode/` directory in project root
- **Naming**: `.opencode/hookify.{descriptive-name}.local.md`
- **Gitignore**: Add `.opencode/*.local.md` to `.gitignore`
