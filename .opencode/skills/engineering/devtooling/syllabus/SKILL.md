...
> **Portability:** Requires a Consensus MCP connection, Node.js with `docx` package, and file reading capability for the syllabus. Works in the coding agent CLI natively. In the agent platform with Consensus MCP + Code Execution + file upload, the workflow is supported.
