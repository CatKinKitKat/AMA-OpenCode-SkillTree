---
name: ecc-the agent-devfleet
description: Orchestrate multi-agent coding tasks via The agent DevFleet — plan projects, dispatch parallel agents in isolated worktrees, monitor progress, and read structured reports.
origin: community
---


# The agent DevFleet Multi-Agent Orchestration

## When to Use

Use this skill when you need to dispatch multiple the coding agent agents to work on coding tasks in parallel. Each agent runs in an isolated git worktree with full tooling.

Requires a running The agent DevFleet instance connected via MCP:
```bash
the agent mcp add devfleet --transport http http://localhost:18801/mcp
```
