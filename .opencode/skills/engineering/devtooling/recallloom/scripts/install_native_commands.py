...
SUPPORTED_HOSTS = ("the agent-code", "gemini-cli", "opencode")
SUPPORTED_SCOPES = ("project", "user")
MANAGED_MARKER_LABEL = "RecallLoom managed native command"
REQUIRED_NATIVE_COMMANDS = {
    "the agent-code": {"rl-init.md", "rl-resume.md", "rl-status.md", "rl-validate.md"},
...
def host_command_dir(host: str, *, scope: str, project: Path) -> Path:
    if host == "the agent-code":
        return (project / ".the agent" / "commands") if scope == "project" else (Path.home() / ".the agent" / "commands")
    if host == "gemini-cli":
...
def detect_dispatcher_command(project: Path, *, scope: str) -> str:
    ...
    candidates = [
        ...
        project / ".the agent" / "skills" / "recallloom" / "scripts" / "recallloom.py",
