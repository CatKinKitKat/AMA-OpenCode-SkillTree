...
- Choosing public bug bounty targets or prioritizing large-vendor programs before testing begins
...
## Source-code bounty triage

When the scope artifact lists public source repositories instead of a live URL, do source-first triage:

1. Resolve the exact repository from the scope item.
2. Read repo-local instructions first (`AGENTS.md`, `AGENTS.md`, or nearer project guidance).
3. Search for high-risk sinks before broad code reading:
   - command execution
   - file read/write/delete
   - path joins and temp-file usage
   - network fetches and redirects
   - host/origin/CSRF comparisons
   - templating and HTML sinks
   - deserialization, eval, and code generation
4. Narrow each hit to the live call path. Ignore test files, fixtures, compiled bundles, and dead comments unless runtime evidence shows they execute.
5. Return only evidence-backed findings: file path, symbol, source-to-sink path, and the trust-boundary failure.
6. If a hit is only a weak lead, keep searching rather than speculating.

See `references/source-code-repo-triage.md` for the compact workflow used in repo-scope sessions. For the Next.js image optimizer SSRF/DNS-rebinding audit pattern, see `references/vercel-nextjs-image-optimizer.md`.
