...
Usage in execute_code:
    exec(open(os.path.expanduser(
        os.path.join(os.environ.get("AGENT_HOME", os.path.expanduser("~/.agent")), "skills/red-teaming/godmode/scripts/auto_jailbreak.py")
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("AGENT_HOME", Path.home() / ".agent")) / "skills" / "red-teaming" / "godmode"
...
# ═══════════════════════════════════════════════════════════════════
# the agent config paths
# ═══════════════════════════════════════════════════════════════════

AGENT_HOME = Path(os.getenv("AGENT_HOME", Path.home() / ".agent"))
...
MODEL_STRATEGIES = {
    ...
    # Nous/the agent models — already uncensored, just needs clean prompt
    "agent": {
        "order": ["prefill_only"],
        "system_templates": {},
    },
...
def _detect_model_family(model: str) -> str:
    ...
    if "the assistant" in model_lower or "anthropic" in model_lower:
        return "the agent"
    ...
    if "grok" in model_lower or "x-ai" in model_lower:
        return "grok"
    if "agent" in model_lower or "nous" in model_lower:
        return "agent"
...
def _get_current_model() -> tuple:
    """Read current model and provider from the agent config.yaml.
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.agent/prefill.json."""
    ...
    if winning_strategy:
        ...
        if not dry_run:
            ...
            if verbose:
                ...
                print()
                print("[DONE] Jailbreak locked in. Restart the agent for changes to take effect.")
        else:
            if verbose:
...
def undo_jailbreak(verbose=True):
    ...
    if verbose:
        print("[UNDO] Jailbreak removed. Restart the agent for changes to take effect.")
