...
- GitHub Actions or similar automation triggered by public issues/comments can create a privileged input surface when the workflow exposes `allowed_non_write_users`, repository tokens, or provider API keys.
- Treat model-facing actions as untrusted until the effective tool scope is proven. Verify any `CLAUDE_CODE_SCRIPT_CAPS`, wrapper scripts, or slash-command restrictions actually confine what the model can invoke.
- Prefer repo-local evidence over workflow intent. A safe-looking wrapper is not enough if the underlying action can still access secrets or dispatch follow-on jobs.
- For issue/comment triggers, check whether non-write users can inject content that reaches comments, labels, dispatch payloads, or downstream scripts.
- See `references/gh-actions-public-input.md` for the compact audit note on public GitHub issue input driving privileged automation.
