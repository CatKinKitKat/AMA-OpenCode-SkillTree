...
engine = PrivacyImpactAssessmentEngine(
    organization_name="the-project Corp",
    dpo_email="dpo@the-project.com",
)
```
...
activity = engine.register_processing_activity(
    ...
    data_controller="the-project Corp",                  # Controller name
...
python agent.py --action demo --org "the-project Corp" --output report.json

# Run screening checklist
python agent.py --action screening

# Specify DPO email
python agent.py --action demo --dpo-email dpo@the-project.com --output dpia.json
```
