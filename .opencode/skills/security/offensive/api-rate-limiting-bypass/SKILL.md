...
## Rate-Limited Automation Without Bypass

When the user chooses normal operation rather than bypass testing, use the bounded pattern in `references/low-frequency-rate-limited-api-automation.md`: identify whether the system is truly on-chain or centralized, obey 429 responses, add minute-scale randomized backoff, filter expensive work, and log every long-running helper action. Do not apply bypass probes to a live service when the task is ordinary participation rather than authorized control validation.
