...
A the coding agent skill that triages a batch of raw security-scanner findings:
...
- the coding agent CLI installed and authenticated
- A read-only checkout of the target codebase (verification reads source. It does not build or run anything)
...
```bash
ls .opencode/skills/triage/SKILL.md
```
...
cp -r .opencode/skills/triage ~/.opencode/skills/
```

## Usage

From a the coding agent session in the target repo:
...
- **Does not:** build, run, or test the target. Install dependencies. Reach the network. Write proof-of-concept exploits. All conclusions are
