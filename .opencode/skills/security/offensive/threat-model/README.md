...
A the coding agent skill that builds a threat model for a target codebase. Two
...
reasoning-capable The agent models: use the same model you run the rest of the
...
```bash
ls .opencode/skills/threat-model/
```

User-scoped:

```bash
cp -r .opencode/skills/threat-model ~/.opencode/skills/
```
...
- Shostack, *The Four Question Framework for Threat Modeling* (2024): https://shostack.org/files/papers/The_Four_Question_Framework.pdf
