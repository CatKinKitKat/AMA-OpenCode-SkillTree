...
> with `cat .opencode/skills/threat-model/bootstrap.md` via Bash.
...
- `--depth recon|full` (optional, default `full`): `recon` runs stages 1-2
  only. Still write all eight sections (schema requires sections 1-7. Section 8 optional). Leave section 4, section 5, and section 8 as header + empty table, and put "run with --depth full
...
`python3 ~/.agent/skills/community/defending-code-reference-harness/_lib/checkpoint.py load ./.threat-model-state`

- `status == "absent"` OR `"complete"`, OR `--fresh` in `$ARGUMENTS` →
  **fresh start.** Bash:
  `python3 ~/.agent/skills/community/defending-code-reference-harness/_lib/checkpoint.py reset ./.threat-model-state`,
...
1. Write tool → `./.threat-model-state/_chunk.tmp` containing the
   stage's output JSON.
2. Bash → `python3 ~/.agent/skills/community/defending-code-reference-harness/_lib/checkpoint.py save ./.threat-model-state <N> <name> --key stage --from ./.threat-model-state/_chunk.tmp`

**End of run.** After writing `<target-dir>/THREAT_MODEL.md`, Bash:
`python3 ~/.agent/skills/community/defending-code-reference-harness/_lib/checkpoint.py done ./.threat-model-state 5 --key stage`
...
Then Bash:
`python3 ~/.agent/skills/community/defending-code-reference-harness/_lib/checkpoint.py save ./.threat-model-state 1 swarm --key stage --from ./.threat-model-state/_chunk.tmp`
...
Then Bash:
`python3 ~/.agent/skills/community/defending-code-reference-harness/_lib/checkpoint.py save ./.threat-model-state 2 synthesize --key stage --from ./.threat-model-state/_chunk.tmp`
...
without overflow checks). You are not trying to prove these are exploitable. You are estimating how much of the surface shares the pattern. More siblings →
higher likelihood.
...
- `actor`: from the entry point (file parsing → whoever supplies the file. Network endpoint → `remote_unauth` or `remote_auth` depending on whether
  auth precedes it).
...
- `controls`: grep for mitigations relevant to the stack (size caps, input
  validation, sandboxing/seccomp. ASLR/stack-protector/CFI in native builds. Parameterized queries / ORM. Auth middleware / CSRF tokens / CSP. Rate
...
Then Bash:
`python3 ~/.agent/skills/community/defending-code-reference-harness/_lib/checkpoint.py save ./.threat-model-state 3 generalize --key stage --from ./.threat-model-state/_chunk.tmp`
...
Then Bash:
`python3 ~/.agent/skills/community/defending-code-reference-harness/_lib/checkpoint.py save ./.threat-model-state 4 gap-fill --key stage --from ./.threat-model-state/_chunk.tmp`
...
2. For each remaining section (`## 2. Assets`, `## 3. Entry points`,
   ...
   - Write tool → `./.threat-model-state/_chunk.tmp` containing that ONE
     section's markdown.
   - Bash:
     `python3 ~/.agent/skills/community/defending-code-reference-harness/_lib/checkpoint.py append ./.threat-model-state/THREAT_MODEL.md --from ./.threat-model-state/_chunk.tmp`
